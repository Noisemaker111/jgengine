# Decision 0002 — Thin `gameContext.ts` by mechanical extraction first; `systems`/`defineSystem` (#842) is a separate, human-greenlit phase

- Status: Accepted
- Date: 2026-07-16
- Deciders: engine
- Informs: decision 0001 (descriptor lifecycle ownership), #842 (composable systems), critique item #11
...
1. **Mechanical extraction (internal only).** Move descriptor bodies into `runtime/descriptors/*`, convert the always-on baseline subsystems into always-on descriptors per decision 0001, collapse `createGameContext` toward the loop. No public API change; `defineGame`, `loop.ts`, and every game in `Games/*` untouched.
...
- **Extend through seams, never edits.** The descriptor contract *is* the seam #842's `defineSystem` needs: #842's acceptance criteria ("systems own save, replication, reset and disposal") are exactly decision 0001's `FeatureInstance` fields. Finishing descriptor ownership first means `defineSystem` becomes a public face over an existing internal contract instead of a second parallel lifecycle model — avoiding the "two authoring paths" flaw #842 itself warns about (its cartridge critique).
...
- Save/replication membership is now structurally owned (decision 0001 §2 realized for serialization); a new baseline subsystem registers one descriptor instead of editing two distant arrays.
...
- #842 remains open and unblocked; its issue tracks the public authoring model, this decision record owns the internal substrate sequencing.
