...
Cartridge-shaped games (declarative config, engine-owned loop): see [reference-cartridge.md](https://github.com/Noisemaker111/jgengine/blob/main/.Codex/skills/jgengine/reference-cartridge.md).

---

The engine ships **verbs and primitives**; your game ships **nouns** (catalogs) and thin handlers. This skill is that foundation plus intake. Use domain skills only when needed; use `jgengine-verify` afterward. UI guidance lives in [`../jgengine-ui/reference.md`](https://github.com/Noisemaker111/jgengine/blob/main/.Codex/skills/jgengine-ui/reference.md); assets live in `jgengine-assets`.

Load detailed references only for the selected domain: [`jgengine-combat`](https://github.com/Noisemaker111/jgengine/blob/main/.Codex/skills/jgengine-combat/reference.md), [`jgengine-world`](https://github.com/Noisemaker111/jgengine/blob/main/.Codex/skills/jgengine-world/reference.md), [`jgengine-multiplayer`](https://github.com/Noisemaker111/jgengine/blob/main/.Codex/skills/jgengine-multiplayer/reference.md), and [`jgengine-ui`](https://github.com/Noisemaker111/jgengine/blob/main/.Codex/skills/jgengine-ui/reference.md). Each domain skill explains when its reference is needed.
...
| Typed store handle | `store/defineStore` | `defineStore<T>(key, initial)` → `StoreHandle<T>` (`read`/`peek`/`write`/`update`/`clear`); pair with `useStore` (`@jgengine/react/store`). The cast-free default for a single fixed slot of run state |
| Typed keyed-store handle | `store/defineKeyedStore` | `defineKeyedStore<T>(keyFor, initial)` → `KeyedStoreHandle<T>` (`read`/`peek`/`write`/`update`/`clear`, all `(ctx, id, ...)`); pair with `useKeyedStore` (`@jgengine/react/store`). The `defineStore` sibling for a per-owner keyed family (per-user, per-instance) |
| Reactive keyed store | `store/observableKeyedStore` | `createObservableKeyedStore`, `ObservableKeyedStore` — backs `ctx.game.store`, sits under `defineStore`/`defineKeyedStore` |
...
- **Keybind → command convention.** The shell fires a command for any bound action that isn't reserved: pressing an action runs a command of the **same name** if one is defined, else a `ui.<action>` fallback (so `openBackpack` → `ui.openBackpack`). Just declare the binding and a matching command — no per-game `keydown` listener. Reserved actions the shell consumes natively and never routes to a command: `moveForward/moveBack/moveLeft/moveRight`, `turnLeft/turnRight`, `sprint`, `jump`, `tabTarget`, `clearTarget`, `useAbility`, `interact`, and any `hotbarSlotN`/`slotN`. `tabTarget`/`clearTarget` run `target.cycle`/`target.clear` (native `cycleTarget`/`setTarget` fallback). **`hotbarSlotN` and a game's own `selectSlotN` are two different hotbar models, not a duplication** (#743): reserved `hotbarSlotN` is *direct-activate* — press `1` and the shell immediately uses/equips slot 1's item (Minecraft/shooter quick-use), so it never reaches a command. A *select-then-act* game (a weapon wheel where `1` picks the active weapon and a separate `fire` uses it) needs persistent selection state the shell doesn't own, so it binds its own `selectSlot1..N` actions, defines commands that write a store key, and reads it back through `hotbarSelection: () => ...` on `defineGame` (see `vice-isle`). Pick `hotbarSlotN` when the number key *is* the action; bind `selectSlotN` only when a later verb consumes the selection.
...
ctx.touch()         bumps ctx.version()/notifies ctx.subscribe directly — the escape hatch for a
                    command that only mutates game-owned state outside entity/object/economy/store
                    (an external session map, a closure variable) and needs a reactive HUD refresh
                    without faking a ctx.game.store.set
```
...
- [ ] HUD screenshotted over a staged `GameUiPreview` scenario and **judged by looking at the image** against the UI quality bar in [`../jgengine-ui/reference.md`](https://github.com/Noisemaker111/jgengine/blob/main/.Codex/skills/jgengine-ui/reference.md) — the final human glance, not the verification loop
