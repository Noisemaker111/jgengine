...
describe("resolveCityLotContent — blockFill interior fill + parks", () => {
  ...
  test("generateCity threads content.blockFill into both the frontage compaction and the interior pass", () => {
    // The plot-contract walk packs enough lots to saturate the default 400-lot cap at this size,
    // so lift the cap for the count comparison — compaction must add lots, not hit the ceiling.
    const compact = generateCity({ seed: SEED, lots: { maxLots: 1500 }, content: { blockFill: 1 } }, HX, HZ);
    const loose = generateCity({ seed: SEED, lots: { maxLots: 1500 } }, HX, HZ);
    // Frontage lots compacted (more of them) AND interiors filled behind them.
    expect(compact.lots.length).toBeGreaterThan(loose.lots.length);
