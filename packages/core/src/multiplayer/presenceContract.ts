...
export interface PresenceActions<TGameId extends string = string> {
  ...
  leavePresence(args: LeavePresenceArgs<TGameId>): Promise<{ left: boolean }>;
...
export function createLocalPresenceTransport<TRow, TLocation, TGameId extends string = string>(): {
  transport: PresenceTransport<TRow, TLocation, TGameId>;
  actions: PresenceActions<TGameId>;
} {
  const feeds: PresenceFeeds<TRow, TLocation> = {
    myPresenceLocation: null,
    onlinePresences: [],
    dormantPresences: [],
  };
  const actions: PresenceActions<TGameId> = {
