...
import { downloadPackArchive, extractGlbs, mirrorOverrideUrl, sha256Hex, type FetchLike } from "./download";
...
describe("downloadPackArchive", () => {
  ...
  test("falls back to the primary provider url when the mirror override fails", async () => {
    const calls: string[] = [];
  ...
  test("falls back to the pack's own mirror when the mirror override and primary both fail", async () => {
    const calls: string[] = [];
  ...
  test("tries the primary url straight away when no mirror base override is set", async () => {
    const calls: string[] = [];
  ...
  test("resolves a scrape-type primary source through its page before falling back", async () => {
  ...
  test("rejects bytes from a mirror that don't match the pinned sha256 and falls through to a source that does", async () => {
  ...
  test("throws an aggregated error naming every attempted url when all sources fail", async () => {
    const source: AssetSource = { ...baseSource, mirror: PACK_MIRROR_URL };
    const fetchImpl = fetchFrom({});
