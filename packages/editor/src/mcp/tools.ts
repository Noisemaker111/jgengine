...
const TOOL_DESCRIPTIONS: { name: EditorBridgeRequest["method"]; description: string }[] = [
  ...
  { name: "add_marker", description: "Author a new point marker into the scene document at x/z (optional y) — the headless one-shot for placing a logical marker (spawn, checkpoint, pickup, bounty, or a game's own custom kind) without an export/import roundtrip. kind is required; color/label/rotationY/catalogId/meta are optional (catalogId names the entity kind a mob/boss marker spawns) and meta is validated against the kind schema when registered. Re-ids on a document-global id collision." },
  { name: "set_marker", description: "Patch a marker's kind/color/label/rotationY/catalogId and merge-patch its meta. Validated against the kind schema when registered." },
  ...
  { name: "bakeNavMesh", description: "Bake indexed scene geometry (walkable faces counter-clockwise seen from above) into a named, agent-sized navigation mesh stored in the document." },
