...
All eight packages version in lockstep. When you bump (e.g. `0.5` → `0.6`) to pick up new capabilities, read [`CHANGELOG.md`](https://github.com/Noisemaker111/jgengine/blob/main/CHANGELOG.md) — each release leads with a **Migrate** block listing the concrete steps to move a game onto the new APIs. It ships inside every package too (`node_modules/@jgengine/core/CHANGELOG.md`), and as typed values: `import { VERSION, CHANGELOG } from "@jgengine/core/meta/changelog"` to diff your installed version against the latest programmatically.
...
| `heightfield({ amplitude, frequency, octaves?, lacunarity?, persistence?, ridged?, baseHeight?, waterLevel?, bounds? })` | Full procedural control (one noise field, valleys→mountains) |
...
**Regions** (`@jgengine/core/world/regions`) — the engine's content-agnostic multi-region mechanism, for biome-style worlds. `createRegionField({ regions, seed, selectorFrequencies?, axisWeights?, seaLevel? })` returns a `RegionField` (which *is* a `TerrainField`). You supply `RegionDef[]`: each has a `selector` (a center in N-dimensional selection space — you decide the axes, e.g. temperature/humidity/continentalness), a `height` (`baseHeight/amplitude/frequency/ridged`), the engine's generic knobs (`tint`, `steepTint`, `water`, `fog`, `fogDensity`, `speedMultiplier`), and an opaque `data: T` payload the engine never inspects. The engine selects the nearest region in selector space with softmax blending — height and colors blend smoothly across borders — and `field.sampleRegion(x, z)` returns the dominant region, blended tint/water/fog, `speedMultiplier`, and your `data`. Set it on the game via `PlayableGame.terrain`; the shell colors the ground by `tint`, tints water by `water`, applies `speedMultiplier` to move speed, and closes fog in by `fogDensity`. The engine holds **no biome/monster/tree content** — names, spawn tables, and prop palettes live in your `data`. See the `biome-showcase` game for a worked ~18-region catalog.

**Scatter** (`@jgengine/core/world/scatter`) — deterministic placement of *opaque item ids*. `scatterItems(field, area, layersFor, { cell?, max? })` walks a grid, asks your `layersFor(sample) => ScatterLayer[]` which items may appear (each `{ item, density, minScale?, maxScale?, maxSlope?, aboveWaterOnly? }`), rolls one, and returns grounded instances (`{ item, x, y, z, scale, rotation, regionId }`). The engine never interprets `item` — map it to a mesh in your `WorldOverlay` or to an entity via `spawn`. `pickWeighted(entries, roll)` is the matching weighted picker for spawn tables.
