...
Pilot items live now: `game-icon` (glyph catalog + `iconForItemId`/`iconForAction`), `vital-bar`, `entity-vital-bar`, `ability-slot`, `keybind-badge`. The npm `gameui` kit above remains available while the rest of the catalog ports over; where a registry item exists, prefer it — and never hand-roll a gray-box version of a component either path already ships.
