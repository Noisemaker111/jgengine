...
**Movable HUD panels** (`@jgengine/react/hudLayout` over `@jgengine/core/ui/hudLayout`) — the draggable-canvas layer that makes a HUD rearrangeable instead of hard-anchored. `useHudLayout({ storageKey?, snap?, locked? })` creates a `HudLayoutStore` (pure core store: anchor-relative placements, z-order, clamp/snap math, `editing`/`locked` flags, serialize/hydrate) and, when `storageKey` is set, persists moved panels to `localStorage` automatically. Wrap the HUD in `<HudCanvas layout={layout}>` (the full-bleed `pointer-events-none` root you would otherwise hand-roll) and put each cluster in `<HudPanel id="minimap" anchor="top-right" inset={{ x: 16, y: 16 }}>` — the panel renders `pointer-events-auto` and positions itself against the nearest of nine viewport anchors so layouts survive resizes. During play the layout is fixed; **holding F2 and pressing C toggles HUD-edit mode** (`HudCanvas` installs the chord, customize or disable via `editChord`; Esc also exits), where every panel becomes draggable with dashed outlines, id labels, a click shield over its content, and a top-center bar with Reset/Done — dragging clamps to the viewport, re-anchors to the nearest region, brings the panel to front, and sets `data-dragging`; `data-hud-editing` is set on the canvas root. Programmatic control: `layout.setEditing(b)`, `layout.reset(id?)`, per-panel `locked` prop to exempt a panel, `locked: false` on `useHudLayout` for tldraw-style always-draggable panels (4px drag threshold keeps buttons and inputs inside panels clicking normally). Core math (`nearestAnchor`, `placementFromRect`, `rectFromPlacement`, `clampRect`, `anchoredPlacement`, `isPanelDraggable`) is pure and browserless-testable.

**Layout rule:** all **screen** positioning (`absolute`, `inset-*`, grid zones, flex regions) lives on wrappers inside `ui/GameUI.tsx` only — either hand-anchored divs or, preferably, `HudCanvas`/`HudPanel` so players can rearrange the HUD. `ui/components/` files are content + hooks only — internal `relative`/`absolute` for bar overlays or slot badges inside a component is fine; never anchor a component to the viewport from a child file. Pass `className` to primitives for **visual** styling (colors, borders, size), not screen placement.
