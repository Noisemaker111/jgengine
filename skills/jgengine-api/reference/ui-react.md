...
**Movable HUD panels** (`@jgengine/react/hudLayout` over `@jgengine/core/ui/hudLayout`) — the draggable-canvas layer that makes a HUD rearrangeable instead of hard-anchored. `useHudLayout({ storageKey?, snap?, locked? })` creates a `HudLayoutStore` (pure core store: anchor-relative placements, z-order, clamp/snap math, serialize/hydrate) and, when `storageKey` is set, persists moved panels to `localStorage` automatically. Wrap the HUD in `<HudCanvas layout={layout}>` (the full-bleed `pointer-events-none` root you would otherwise hand-roll) and put each cluster in `<HudPanel id="minimap" anchor="top-right" inset={{ x: 16, y: 16 }}>` — the panel renders `pointer-events-auto`, positions itself against the nearest of nine viewport anchors so layouts survive resizes, and is draggable by default with a 4px movement threshold so buttons and inputs inside it still click normally. Dragging clamps to the viewport, re-anchors to the nearest region, and brings the panel to front; `data-dragging` is set while moving. Opt a single panel out with `locked`, freeze the whole layout with `layout.setLocked(true)` (WoW-style lock/unlock), restore defaults with `layout.reset(id?)`. Core math (`nearestAnchor`, `placementFromRect`, `rectFromPlacement`, `clampRect`, `anchoredPlacement`) is pure and browserless-testable.
