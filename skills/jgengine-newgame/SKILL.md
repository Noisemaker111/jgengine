...
- **Terrain**: `world:` now drives a real height field. Pick a profile from `@jgengine/core/world/features` — `flat()` (true flat + grid), `rolling()` (subtle imperfections, the default for an undeclared `world`), `arena()` (flat spawn + a walkable ramp up to a cliff plateau + a hill — the shipped test environment), or `heightfield({ amplitude, frequency, octaves, ridged, waterLevel })` for full control. The shell renders the mesh from this field and **grounds every entity/object/rock on it automatically**, so spawns authored at `y:0` sit on the surface — no more clipping into hillsides. To sample ground height yourself (e.g. to place something at a known altitude), call `terrainFieldFor(game.world).sampleHeight(x, z)` from `@jgengine/core/world/terrain`.
