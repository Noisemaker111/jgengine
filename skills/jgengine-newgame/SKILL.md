...
- **Ground** — textured/material'd terrain or tiles with variation (Poly Haven / ambientCG per `jgengine-assets`), never the default grid or a flat color. If `world` is an `environment()` descriptor, leave `PlayableGame.environment` unset and the shell auto-renders it as the backdrop; supply `PlayableGame.environment` (a canvas component) yourself only for a hand-authored or non-`environment()` scene. Either way it replaces the default ground plane + debug grid + rock field.
