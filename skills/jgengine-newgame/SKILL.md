...
- **Terrain**: `world:` now drives a real height field. Pick a profile from `@jgengine/core/world/features` — `flat()` (true flat + grid), `rolling()` (subtle imperfections, the default for an undeclared `world`), `arena()` (flat spawn + a walkable ramp up to a cliff plateau + a hill + a water basin — the shipped test environment), `heightfield({ amplitude, frequency, octaves, ridged, waterLevel })` for full control, or `biomes({ seed })` for a full procedural multi-biome overworld (24-biome catalog with per-biome terrain, surface/water color, monster spawn tables, structures, and player effects — see `jgengine-api`). The shell renders the mesh from this field (procedural or per-biome shading, and a water plane when `waterLevel`/sea level applies) and **grounds every entity/object/rock on it automatically**, so spawns authored at `y:0` sit on the surface — no more clipping into hillsides. The controller follows terrain: the avatar walks up ramps, **falls off ledges under gravity**, and steep faces block it. To sample ground height or the biome yourself, call `terrainFieldFor(game.world)` from `@jgengine/core/world/terrain` — `.sampleHeight(x, z)` and, for biome worlds, `.sampleBiome(x, z)`. The `biome-showcase` dev game (`?game=biome-showcase`) is a live reference.
