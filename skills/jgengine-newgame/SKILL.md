...
- **Terrain**: `world:` drives a real height field. Pick a profile from `@jgengine/core/world/features` — `flat()` (true flat + grid), `rolling()` (subtle imperfections, the default for an undeclared `world`), `arena()` (flat spawn + a walkable ramp up to a cliff plateau + a hill + a water basin — the shipped test environment), or `heightfield({ amplitude, frequency, octaves, ridged, waterLevel })` for full procedural control. For a biome-style world, build a `createRegionField({ regions })` (`@jgengine/core/world/regions`) and set it on `PlayableGame.terrain` — the engine blends per-region height/tint/water/fog/speed and exposes your opaque per-region `data`; scatter props/mobs with `scatterItems`/`pickWeighted` (`@jgengine/core/world/scatter`). The engine ships **no biomes/trees/monsters** — that content is yours (see the `biome-showcase` game for a worked catalog + prop meshes). The shell renders the mesh (procedural or per-region shading, water plane at sea level) and **grounds every entity/object on it automatically**, so `y:0` spawns sit on the surface — no clipping. The controller follows terrain: walk up ramps, **fall off ledges under gravity**, steep faces block you. Sample yourself via `terrainFieldFor(game.world)` / your region field — `.sampleHeight(x, z)` and, for region fields, `.sampleRegion(x, z)`.
