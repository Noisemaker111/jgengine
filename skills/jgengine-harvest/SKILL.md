...
| A preset for a setup exactly one game has used | Recipe in `jgengine-workflow`; let the second game promote it to code |
...
| Report proposes a primitive, never says to delete the game's workaround | The Definition of done requires consuming it in the dogfood game and deleting the glue — that's the proof and the regression guard |
