...
3. **Wire through the engine seams.** GLB models go in the game's `assets.ts` render catalog keyed by catalog id; billboarded characters/enemies use `PlayableGame.entitySprites`; ground materials and skies belong to the world layer. Catalog `model` fields reference asset keys — never file paths scattered through game logic.
