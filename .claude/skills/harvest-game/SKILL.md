...
6. **Session end.** File the gaps as `[FEATURE]` issue(s) on this repo — title `[FEATURE] <brief summary>`, body a numbered list of the raw engine problems (nothing else, no game context, no solutions). Then push, open the PR (`gh pr create --fill`), and queue the merge with `gh pr merge --squash --auto` — never poll CI: it runs the same gate you already ran locally and auto-merge lands the PR the moment CI goes green (~30s). The one exception is a red run: launch a single `gh pr checks <n> --watch --fail-fast` as a background task to get notified, never a polling loop.
