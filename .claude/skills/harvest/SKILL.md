...
7. **Ship in one motion.** Push, open the PR (GitHub MCP `create_pull_request`, ready for review), squash-merge it immediately (`merge_pull_request`) — the local gate already proved what CI would. If a fixed `[FEATURE]` gap issue was your carrier or you closed one inline, put `Closes #N` in the PR body. If required checks block the instant merge, queue `enable_pr_auto_merge` (squash) and end the turn — never subscribe, watch, or poll CI. Echo 🚀 after the merge (chat only, never on GitHub).
