...
- Recorded clips: `bun run drive <game> --record <name>` records every rendered frame of the whole drive (CDP screencast) into `shots/<game>-<name>.gif` as an **animated GIF** — the one clip format that animates everywhere (GitHub PR bodies via the camo proxy, chat panels, editors; animated PNG/WebP show as stills in too many viewers) — and auto-thins frames to stay under ~4.5MB, the ceiling GitHub's camo image proxy will still serve, which is what lets the clip animate inline in a PR body via `pr-shots` raw URLs. The same run also writes `shots/<game>-<name>.mp4`: the full-fidelity real video — every captured frame, true timing, no size cap or thinning — push it with `pr-shots` too, which links it for download. For GitHub's NATIVE inline player, `bun run pr-video <clip.mp4>` uploads to GitHub's user-attachments store and prints a URL that — pasted on its own bare line in the PR body — renders as a real video player; it requires the `GH_SESSION_TOKEN` env secret (a github.com `user_session` cookie; there is no token API for attachment uploads — see the script header for setup). Without that secret, the GIF is the inline evidence and the mp4 stays a download link; do not chase any other inline-video path, none exists. Record at `--size half` (a quarter of the pixels renders several times more frames on software GL) and shrink `--record-width` before lengthening a drive that gets thinned to a slideshow. A clip is prosecuted like any screenshot: extract or step through frames, and pair it with `--probe` deltas — the probe proves the physics, the clip shows it.
