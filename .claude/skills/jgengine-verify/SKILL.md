...
- **A start/title screen blocks `--mode play`? Dispatch past it with `--run`.** `bun run shoot <game> --mode play --run start,deal` runs each named command through `ctx.game.commands.run` right after boot, before the capture handshake — so a menu-gated game screenshots its live world with no store-default flipping or UI restructuring. Add `--settle <ms>` to wait past an intro cinematic before capture. Unknown command names are skipped silently, so name them exactly as the game registers them.
