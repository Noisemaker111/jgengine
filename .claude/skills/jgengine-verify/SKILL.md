...
Any PR that changes what a player sees (world look, HUD, menus, previews, art direction) embeds its final screenshots in the PR body — the reviewer judges pixels, not prose. Mechanism: upload each PNG with GitHub MCP `create_or_update_file` to the **`pr-shots`** branch (create it from `main` if missing; it is an archive, never merged) at `pr-shots/<pr-branch>/<name>.png`, then embed in the PR body as `![name](https://raw.githubusercontent.com/Noisemaker111/jgengine/pr-shots/pr-shots/<pr-branch>/<name>.png)`. Desktop + mobile when the HUD is the point. Non-visual PRs skip this entirely.
