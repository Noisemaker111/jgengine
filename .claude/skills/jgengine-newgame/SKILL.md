...
- **Site the world with `position`**: several `building({ position, count, seed, style })` clusters in one `environment()` are distinct settlements — vary `style` per cluster (a capital vs. a frontier outpost vs. a coastal port) so they read distinctly at a glance, not just by position; buildings ground-snap to the terrain field. Sample the same `resolveTerrainField` for anything the game moves across the terrain — the field the shell renders is the field you query.
