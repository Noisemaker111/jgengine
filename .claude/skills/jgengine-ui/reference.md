...
**Settings menu (auto-mounted, fully customizable).** Every game gets a gear-button settings menu for free — Sound (master + per-bus volume), Graphics (quality/dpr + shadows), Gameplay (FOV slider, default 40â€“120), Controls (per-action key rebinding, persisted). No wiring: it reads the game's `audio.buses` and `input` map. Tune it via `defineGame({ settings })` (`GameSettingsConfig` from `@jgengine/core/settings/settingsModel`):

- `mode: "overlay" | "page"` — over-screen modal vs full-page navigation.
- `surface: "quick"` — swap the gear for compact on-screen volume/graphics buttons. `surface: false` — mount no engine trigger (you open it yourself, see below). `settings: false` — off entirely.
...
**Present it any way you want.** `useSettings()` (`@jgengine/react`) returns the live controller — `{ categories, isOpen, open, close, setOpen, mode, surface }` — so a game can wire its own pause-menu/gear to `open()`, or render `categories` (rows carry `value`/`set`/bounds, keybinds carry `rebind`/`reset`) entirely inside its own HUD and never use the engine chrome (`surface: false`). Read/write a single value with `useSetting(id, fallback)`. Set a slider's `min`/`max` explicitly â€” an omitted range collapses the thumb to 0/1.
