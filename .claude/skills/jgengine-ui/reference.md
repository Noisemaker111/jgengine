...
**Settings menu (auto-mounted).** Every game gets a gear-button settings menu for free — Sound (master + per-bus volume), Graphics (quality/dpr + shadows), Gameplay (FOV slider, default 40â€“120), Controls (per-action key rebinding, persisted). No wiring: it reads the game's `audio.buses` and `input` map. Tune it via `defineGame({ settings })` (`GameSettingsConfig` from `@jgengine/core/settings/settingsModel`): `mode: "overlay" | "page"` (over-screen vs full-page), `surface: "quick"` swaps the gear for compact on-screen volume/graphics buttons, `surface: false` (or `settings: false`) opts out, `extra: GameSettingDef[]` appends game rows to any category (`onChange(value, ctx)`), `hide` drops built-in categories. Read a setting in game code with `useSetting(id, fallback)` (`@jgengine/react`). Set a slider's `min`/`max` explicitly â€” an omitted range collapses the thumb to 0/1.
