...
- `MinimapChrome` (function): function MinimapChrome({ zoneLabel, clock, showCompass = true, compassProps, headerClassName, zoneLabelClassName, clockClassName, compassClassName, children, ...minimapProps }: MinimapChromeProps): ReactNode — Composed circular-minimap chrome: optional zone-label + clock header above the `Minimap`, optional `Compass` strip below. Purely a wiring layer over the existing primitives — the game supplies the zone name and clock text, this only places them; omit either slot and the header disappears.
- `MinimapChromeProps` (interface): interface MinimapChromeProps extends MinimapProps — Props for `MinimapChrome` — all `MinimapProps` plus the zone-label/clock header and compass slots.
