...
- `RadialVectorOptions` (interface): interface RadialVectorOptions — Options for {@link radialIndexFromVector}.
...
- `radialIndexFromAngle` (function): function radialIndexFromAngle(angle: number, count: number): number — Map an angle (radians from up, clockwise) to the nearest of `count` slice indices.
- `radialIndexFromVector` (function): function radialIndexFromVector(dx: number, dy: number, count: number, options: RadialVectorOptions = {}): number | null — Pick the slice a pointer/stick vector points at. `dx`/`dy` are screen-space (y down): up is `(0, -1)`. Returns null inside `deadZone`, so a centered stick or click selects nothing.
- `radialSlicePosition` (function): function radialSlicePosition(index: number, count: number, radius: number): { x: number; y: number } — Unit-circle position of a slice center (up = `(0, -1)`), scaled by `radius` — where to place a wedge's icon/label.
- `radialSlices` (function): function radialSlices(count: number): RadialSlice[] — Evenly divide a ring into `count` wedges, each centered on its index — the geometry a radial/quick menu (weapon or emote wheel) renders. Angles run from "up" (−Y) clockwise, so index 0 sits at the top.
