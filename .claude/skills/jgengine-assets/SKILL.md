...
|-----------|-----|-----------------|
| Starter figure/prop (people/props/nature/urban) | `createStarterCatalog()` + pull starter packs | `entityModels: { guest: "asset:person_casual" }` (`STARTER_PACKS`) |
| A 3D model (tree, astronaut) | `assets add astronaut --dir ../../apps/dev/public` | `catalog.resolve("kaykit-adventurers/Rogue")!.url` in an `entityModels`/`objectModels` seam |
| A whole style pack | `assets add nature --dir ../../apps/dev/public` | any pulled id (browse with `assets list --source quaternius-stylized-nature`) |
...
| Bulk item/ability icons or UI sprites, not just one glyph | `assets add "game icons" --dir ../../apps/dev/public` | `buildSpriteCatalog({ basePath: "/sprites" }).resolve("gameicons-icons/sword")!.url` — ~4,000 CC BY 3.0 game-icons.net icons in one pull |
...
| Source | What you get |
|--------|--------------|
| [Quaternius](https://quaternius.com) | The go-to broad CC0 low-poly library — hundreds of models across medieval village, sci-fi, city, nature, fantasy props, plus **rigged + animated** base characters, animals, and monsters |
| [KayKit](https://kaylousberg.itch.io) | CC0 low-poly packs incl. **rigged + animated characters**: adventurers, skeletons, dungeons, medieval hexagon tiles |
...
| [freesound CC0 filter](https://freesound.org) | Hit sounds, UI clicks, ambience |

**Rules:**

1. **One style family per game.** Quaternius + KayKit low-poly mix fine; low-poly models on photoreal PBR ground reads broken. Name the family in the blueprint.
...
3. **Wire through the engine seams.** GLB models live in the game's `src/game/assets.ts` render catalog keyed by catalog id; billboards via `entitySprites`, real meshes via `entityModels`/`objectModels` in `defineGame({...})`; ground/skies belong to the world layer. Catalog `model` fields reference asset keys — never file paths in game logic. A pulled PBR material (`buildMaterialCatalog(...).resolve(id)!.maps`) goes onto the ground via `terrain({ detail: { material: { maps, repeat? } } })` (blends with the procedural rock/sand/snow shader) or onto an untextured GLB via that model's `ModelConfig.material.maps` — never a raw `material.map = texture` hack in game code; `assets add <name> --kind material` prints both wirings. The fast path is **`assets add <query>`** (see "One command" above): it pulls + reindexes the pack and prints this exact wiring. Under the hood it's **`@jgengine/assets`** — `buildCatalog({ basePath })` → resolve ids/aliases → urls, with `pull` landing packs in your app's `public/models/` (extracting the pack's shared `Textures/` alongside the GLBs so models render textured). Network-restricted: `pull`/`add` fall back through `--mirror <baseUrl>` / `JGENGINE_ASSETS_MIRROR`, and `--offline` fails fast — see the package README for the fallback order.
...
8. **Scatter regions get the same catalog seam.** `<AuthoredScene scatterModels={{ pine: "quaternius-stylized-nature/Pine_1" }} assets={assets} />` resolves a foliage palette item to a real GLB, GPU-instanced per chunk; unmapped items keep `InstancedScatter`'s stylized proxy (tree/pine/oak/bush/shrub/rock/stone/grass). Keyed by scatter `item`, not entity name — pass a dedicated map even when an id also appears in `entityModels`.
9. **No single "finished X" GLB? Assemble one from `parts`.** A modular kit (a Quaternius medieval village, a KayKit dungeon) rarely ships a complete "castle" or "tower" mesh — it ships wall/tower/roof/gate pieces meant to stack. A `ModelConfig` takes `parts: [{ model, position?, rotation?, scale? }]` — static children at fixed local offsets under the parent, no bone/rig involved (that's `attachments`, above). Each `part.model` (a catalog id or inline `ModelConfig`) resolves through the same catalog and gets its own `dims`/anchor centering, so scale/pivot sanity (rule 5) applies per piece. Picking the single best-matching kit piece and shipping that throws away detail the pack already has — reach for `parts` instead: a tower base with a mid section and a roof stacked on top via `position`.
10. **Declare a parametric asset's front — never hand-tune `rotationY` against nothing.** A `registerAssetGenerator` generator (bookcase, building) builds its front toward **+Z** by convention (`DEFAULT_FORWARD`, `@jgengine/core/scene/facing`) and can declare a different axis via `GeneratedAsset.forward: readonly [number, number, number]` when its geometry doesn't. A product-shot stage reads it to auto-orient — see `jgengine-ui` → "Cinematic studio shots" for `StudioStage`'s `faceCamera`. Eyeballing a `rotationY` until the front happens to face the camera is the smell this closes (#816).
