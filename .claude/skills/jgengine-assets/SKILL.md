...
8. **Scatter regions get the same catalog seam.** `<AuthoredScene scatterModels={{ pine: "kenney-nature/tree_pineDefaultA" }} assets={assets} />` resolves a foliage palette item to a real GLB, GPU-instanced per chunk; unmapped items keep `InstancedScatter`'s stylized proxy (tree/pine/oak/bush/shrub/rock/stone/grass). Keyed by scatter `item`, not entity name — pass a dedicated map even when an id also appears in `entityModels`.

9. **No single "finished X" GLB? Assemble one from `parts`.** A modular kit (Kenney Castle, Tower Defense) rarely ships a complete "castle" or "tower" mesh — it ships wall/tower/roof/gate pieces meant to stack. A `ModelConfig` takes `parts: [{ model, position?, rotation?, scale? }]` — static children at fixed local offsets under the parent, no bone/rig involved (that's `attachments`, above). Each `part.model` (a catalog id or inline `ModelConfig`) resolves through the same catalog and gets its own `dims`/anchor centering, so scale/pivot sanity (rule 5) applies per piece. Picking the single best-matching kit piece and shipping that throws away detail the pack already has — reach for `parts` instead. See tower-guard's keep (`Games/tower-guard/src/game/models.ts`): a `tower-square-base` with `tower-square-mid-windows` and `tower-square-roof` stacked on top.
