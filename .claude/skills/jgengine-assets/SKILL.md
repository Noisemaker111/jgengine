...
|-----------|-----|-----------------|
| A 3D model (tree, alien) | `assets add pine --dir ../../apps/dev/public` | `catalog.resolve("nature/tree_pine")!.url` or `quaternius-stylized-nature/Pine_1` / `kaykit-adventurers/Rogue` |
...
| Source | What you get |
|--------|--------------|
| [Quaternius](https://quaternius.com) | Go-to broad CC0 low-poly library — nature, medieval village, modular scifi, street/city. Free Standard zips pull via OpenGameArt; `extractGlbs` packs glTF→GLB. Ids: `quaternius-stylized-nature/Pine_1`, `quaternius-modular-scifi/Alien_Cyclop` |
| [KayKit](https://kaylousberg.itch.io) | CC0 **rigged characters** + dungeon/furniture/city/space — GitHub mirrors (`KayKit-Game-Assets`). Ids: `kaykit-adventurers/Rogue`, `Knight`, `Mage`, `Barbarian` |
...
8. **Scatter regions get the same catalog seam.** `<AuthoredScene scatterModels={{ pine: "nature/tree_pine" }} assets={assets} />` (or `quaternius-stylized-nature/Pine_1`) resolves a foliage palette item to a real GLB, GPU-instanced per chunk; unmapped items keep `InstancedScatter`'s stylized proxy (tree/pine/oak/bush/shrub/rock/stone/grass). Keyed by scatter `item`, not entity name — pass a dedicated map even when an id also appears in `entityModels`.

9. **No single "finished X" GLB? Assemble one from `parts`.** A modular kit (Quaternius medieval village, KayKit dungeon) rarely ships a complete "castle" or "tower" mesh — it ships wall/tower/roof/gate pieces meant to stack. A `ModelConfig` takes `parts: [{ model, position?, rotation?, scale? }]` — static children at fixed local offsets under the parent, no bone/rig involved (that's `attachments`, above). Each `part.model` (a catalog id or inline `ModelConfig`) resolves through the same catalog and gets its own `dims`/anchor centering, so scale/pivot sanity (rule 5) applies per piece. Picking the single best-matching kit piece and shipping that throws away detail the pack already has — reach for `parts` instead.
