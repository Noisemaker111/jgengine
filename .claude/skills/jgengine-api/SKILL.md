...
| Minimap math | `world/minimap` | `projectToMinimap`, `unprojectFromMinimap`, `clampToMinimapEdge`, `compassBearing`, `headingToBearing`, `bearingToCardinal`, `relativeBearing`, `MinimapView` — `unprojectFromMinimap` is the rotate-aware pixel→world inverse for click-to-pin (#285.6) |
| Map layers | `world/mapLayers` | `MapRoute`, `MapZone`, `MapCellStates`, `pointInMapZone`, `mapLayerColor`, `MAP_LAYER_TONE_COLORS` — renderer-free zone/route/heatmap overlay data with live + dashed-`forecast` states; the react `Minimap`/`WorldMap` render it and `pointInMapZone` answers the same data for rules (#285.1-2) |
...
| Analog axis input | `input/axisInput` | `AxisInput`, `AxisChannel`, `AxisBindingMap`, `DRIVE_AXIS_BINDINGS`, `clampAxis`, `rampToward`, `NEUTRAL_AXIS`, `createAxisChannel`, `GenericAxisChannel` — an `AxisBinding.pointer` source steers the axis from `ctx.input.pointer()` via `sample(dt, isDown, pointer)`; `createAxisChannel` is the same held-key ramping for ANY axis schema (drone pitch/strafe, mech torso) with per-axis ranges (#282.7). `@jgengine/shell/input/mouseLook` (`createMouseLookTracker`) is the pointer-lock mouse-look service for chase/orbit games (#282.8) |
...
| Traversal (grapple/glide) | `physics/traversal` | `Grapple`, `GrappleConfig`, `Glide`, `GlideConfig` — PhysicsWorld rigid bodies only; for the walk controller / kinematic movers use `movement/grappleSwing` + `movement/glideModel` |
| Grapple swing (kinematic) | `movement/grappleSwing` | `createGrappleSwing`, `GrappleSwing`, `GrappleSwingConfig` — rope-constraint step for kinematic controllers: feed position/velocity through `step`, commit the result via `beforeCommit`/`setPose`; pendulum swing without a PhysicsWorld (#282.5) |
| Glide model (kinematic) | `movement/glideModel` | `createGlideModel`, `GlideModel`, `GlideModelConfig`, `GlideInput` — yaw + powered climb + turbulence-degraded control, composing external currents (`flowTube`, `windZones`) per tick (#282.6) |
...
| Vehicle body | `physics/vehicleBody` | `createVehicleBody`, `VehicleBody`, `VehicleBodyConfig`, `WheelSpec`, `GripCurve`, `sampleGripCurve`, `DEFAULT_GRIP_CURVE` — full PhysicsWorld sim with wheels/suspension |
| Kinematic vehicle | `physics/kinematicVehicle` | `createKinematicVehicle`, `KinematicVehicle`, `KinematicVehicleTuning`, `KinematicVehicleStep` — the pure arcade car (steer-yaw, throttle/brake, grip-curve lateral bleed) with `surfaceFriction`/`dragAt` hooks; never hand-roll the racer integration loop (#282.1) |
...
**Player movement tuning** — the `movement` field (`PlayerMovementConfig`) tunes the shell's local-player walk controller beyond `physics.gravity`/`jumpVelocity`: `mode` (`"free"` camera-relative default, `"axis"` locks travel to one world `axis`, `"grid"` snaps each committed step to `cellSize` centers), `collideObjects` (collide against placed scene objects as unit-box AABBs even without `collision.voxel`), and `beforeCommit(frame)` — an escape hatch called each frame with `{ entityId, current, next, dt, ctx }` that can return a replacement `[x, y, z]` to constrain or redirect the step (rails, bounds, custom collision) before it commits and before `onTick` runs; `frame.ctx` is the live GameContext (#282.2), so movement rules read stores/stats directly — never build module-level bridge objects. `canSprint: (ctx) => boolean` gates the built-in sprint multiplier behind a stamina/resource check (#282.3).
...
ctx.player          userId, isNew, inventory, stats (modifiers), loadout,
                    applyLoadout, movement (pose/aim), motion (impulse/pushHorizontal/setVerticalVelocity/setY/takePending
                    — pushHorizontal is the knockback/dash seam: kicks compose with the walk controller and decay, #282.4),
...
movement: {
  ...
  beforeCommit?: (frame: MovementCommitFrame) => readonly [number, number, number] | undefined | void;   // frame = { entityId, current, next, dt, ctx } — intercepts each frame's resolved position before the pose commits; return a replacement to constrain/redirect the step
  canSprint?: (ctx: GameContext) => boolean;   // gates the built-in sprint multiplier behind live game state (stamina, encumbrance)
}
```
