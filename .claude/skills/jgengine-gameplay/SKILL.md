...
`createSaveStore` is for a blob you name yourself. When you want to persist the **entire world** without listing fields, turn on `defineGame({ persist: true })` (offline/single-player only). The engine binds `ctx.game.save` (`@jgengine/core/runtime/runtimeSave`, `RuntimeSave`) to a `localStorage` backend and, in the default `"autosave"` mode, writes the whole live world on a debounce after any change — every `defineStore` slot, all entities + their stats, player inventories, **and the persistent progression subsystems (economy wallets, quest journal, unlocks, roster)** — through the same `SnapshotModule` seam. The game never lists what to save. The save set is a **superset** of the host→client replication set: `ctx.snapshot()`/`ctx.hydrate()` (the multiplayer payload) deliberately omit those progression subsystems, so turning on `persist` never changes what a host replicates.
