...
`createSaveStore` is for a blob you name yourself. When you want to persist the **entire world** without listing fields, turn on `defineGame({ persist: true })` (offline/single-player only). The engine binds `ctx.game.save` (`@jgengine/core/runtime/runtimeSave`, `RuntimeSave`) to a `localStorage` backend and, in the default `"autosave"` mode, writes the whole live world on a debounce after any change — every `defineStore` slot, all entities + their stats, and player inventories, captured through the same `ctx.snapshot()`/`ctx.hydrate()` seam multiplayer replicates over. The game never lists what to save.
