...
### Save the *whole* game automatically — `defineGame({ persist })` + `ctx.game.save`

`createSaveStore` is for a blob you name yourself. When you want to persist the **entire world** without listing fields, turn on `defineGame({ persist: true })` (offline/single-player only). The engine binds `ctx.game.save` (`@jgengine/core/runtime/runtimeSave`, `RuntimeSave`) to a `localStorage` backend and, in the default `"autosave"` mode, writes the whole live world on a debounce after any change — every `defineStore` slot, all entities + their stats, player inventories, **and every stateful subsystem the game opted into** — economy wallets, quest journal, unlocks, roster, cosmetic loadouts, card piles, turn-loop position — plus the always-on baseline (the sim clock's game-time/pause/speed, entity pose & aim, entity possession, per-player motion intents) — through the same `SnapshotModule` seam. The game never lists what to save. Save coverage is **owned per subsystem, not hand-listed**: each opt-in feature descriptor emits its own `save` module, so a newly added persistent feature can't silently fall out of the whole-world save. The save set is a **superset** of the host→client replication set: `ctx.snapshot()`/`ctx.hydrate()` (the multiplayer payload) deliberately omit all of it beyond the replicated baseline, so turning on `persist` never changes what a host replicates.

Config: `persist: true` or `persist: { mode?: "autosave" | "manual", storage?: "local" | "memory", version?, autosaveMs? }`. **The game owns *when* it saves and restores** — that's how one API covers every save mechanic:

- **Continuous autosave** — `mode: "autosave"` (default). Nothing else to write.
- **Save points / manual saves** — `mode: "manual"`, then call `ctx.game.save.checkpoint()` at a save station, on quest-complete, on area-enter — any trigger. `checkpoint()` is an explicit `save()`.
- **Restore on boot** — call `await ctx.game.save.load()` in your loop's `onNewPlayer` (returns `true` when a save was applied and the world is now hydrated, `false` for a fresh start) and branch: resume vs. fresh spawn. Note module-level singletons (a `let` in a game module) are **not** in the world snapshot — re-derive them from the restored `defineStore` slots after `load()`.
- **Save slots** — `switchSlot(id)` / `slots()` / `hasSave()` back a save/load menu and a title-screen "Continue".

Offline↔cloud is the same one-line swap as `createSaveStore`: the shell can inject a Convex backend via the `createGameContext({ save })` seam instead of the default `localStorage` one — game code unchanged. `createRuntimeSave({ target, backend, mode, ... })` is the underlying bridge if you need to wire it by hand (`target` is any `{ snapshot, hydrate, subscribe }` — a `GameContext` satisfies it). The save captures economy/quest/unlocks/roster/cosmetics/cards/turn plus the clock, pose, possession and motion baseline automatically now (each via `snapshotAll`/`hydrateAll`, `snapshot`/`hydrate`, or `capture`/`restore` on its subsystem) — the only state left out of the snapshot is a **module-level singleton** (a bare `let` in a game module), which escapes both save and replication: re-derive it from the restored `defineStore` slots/stats after `load()` (see the-robots' `resumeBuild`, which rebuilds its `activeCharacter`/`talentTree` singletons from the restored `characterId`/`talentRanks` stores). Note: card piles and turn loops restore into piles/loops the game **re-creates on boot** (via `ctx.game.cards.pile(id, config)` / `ctx.game.turn.loop(id, config)`), so register them in setup before calling `load()`.
...
`ctx.game.commands.define(name, { decode?(input), validate?(ctx, input), apply(ctx, input) })` registers a verb (`has`/`names`/`run` round it out); `run(name, input)` returns `{ status: "applied", state } | { status: "rejected", reason } | { status: "unknown-command" }`. `apply` may either **return** the next state (the classic reducer shape) or mutate `ctx` in place and return **nothing** — `run` keeps the current `ctx` as `state` when `apply` returns `void`, so a handler that only calls other `ctx` methods (spawn, effect, loot.grantToPlayer, …) doesn't need a pointless `return ctx`. `input` at the command boundary is raw network `unknown` from a client's `runCommand` — declare `decode(input): { ok: true, value } | { ok: false, reason }` to parse and reject a malformed payload before `validate`/`apply` ever see it (an undeclared `decode` keeps passing input through unchanged, so existing commands are unaffected); never hand-cast `input as { ... }` inside `apply` for anything a client sends. **Event handlers use `ctx` directly** the same way (side effects: leaderboard, economy, scheduling) and never reassign state. One feed primitive for kill feeds, loot logs, quest updates — no per-domain feed hooks.
