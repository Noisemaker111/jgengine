...
- `gamePhase` (function): function gamePhase(ctx: GameContext): GamePhase — Current phase; defaults to `playing` when unset so `lifecycle: "always-live"` games need no wiring. A game never leaves this contract implicit — the game-shape gate requires every game to declare a `lifecycle` (config or `"always-live"`) or publish via {@link setGamePhase}.
