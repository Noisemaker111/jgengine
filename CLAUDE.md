...
- Package manager: bun (workspaces: `packages/*`, `packages/games/*`, `Games/*`, `apps/*`, `examples/*`).
...
- `packages/games/*` — legacy example games (`@dogfood/<name>`, private, source-consumed via `./src` exports, no build). New games go in `Games/`, not here.
- `Games/` — the only place games get built, one directory per game. Any game work anywhere in the repo follows `Games/CLAUDE.md`: engine gaps go straight into `Games/TODO.md` as raw problems, committed and filed as one `[FEATURE]` issue at session end.
