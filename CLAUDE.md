...
- **Never poll CI.** CI finishes in ~30s and runs the same gate you already ran locally — its outcome is known before the PR opens. Auto-merge means no session ever waits for it: queue the merge and move on to the next batch or end the turn. Never loop on `gh pr checks`, never sleep between checks. The one exception is debugging a red run: launch a single `gh pr checks <n> --watch --fail-fast` as a background task and get notified — one call, not a loop.
