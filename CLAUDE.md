...
- **Log papercuts the moment they happen — no exceptions.** Any small friction (retried call, dead-end command, misleading error, flaky worker, wording that steered you wrong) gets `bun run papercut -m <model> "doing X → Y got in the way"` right then. One command, zero ceremony: the entry rides along in whatever ships next — never skip one because it "would need its own PR", the work is mid-flight, or it feels too small. Not `CHANGELOG.md` (what shipped), not an issue (real bugs). `/papercut` sweeps a whole session — user-triggered only.
