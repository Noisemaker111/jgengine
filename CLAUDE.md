...
- **Ship in one motion: push → PR → merge, then stop.** The local gate (via workers) already proved what CI would. When the work is genuinely done and clean (never over doubt): push, open the PR (GitHub MCP `create_pull_request`, ready for review), and squash-merge it immediately (`merge_pull_request`). If GitHub refuses because required checks are still pending, queue it with `enable_pr_auto_merge` (squash) and end the turn — no subscribing, no watching, no polling, no babysitting; CI on `main` is the safety net. Echo 🚀 in your reply after the merge — chat signal only, never on GitHub.
