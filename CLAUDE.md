...
- When the task is **complete**, the **agent** ships it — never ask the user to merge. Run `bun .claude/scripts/ship-pr.mjs`: it pushes, squash-merges, deletes the remote branch, and **returns the primary checkout to `main`**. Then `ExitWorktree` (remove) to drop the worktree. The script prints 🚀 on a successful merge and you should **echo 🚀 in your reply** — the rocket is a chat/session signal that an agent merged; it does **not** go in the PR title or anywhere on GitHub. Only skip the merge if the work is genuinely unfinished or you were told to hold.
