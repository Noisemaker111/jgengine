...
- Keep the PR in a mergeable, reviewable place at all times, so shipping is one click. The **user** presses merge (unless they explicitly hand it off) and the **user** owns publish/version timing — merging to `main` may trigger the publish workflow, so never bump a package version to force a release unless asked. You don't need to babysit CI or gate on it mid-task; surface a red check if you happen to see one.
