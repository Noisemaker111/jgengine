...
- **Each session works in its own worktree — that's the one hard rule.** The primary checkout stays on `main` at all times; work happens in an isolated worktree (`EnterWorktree`, which branches into `.claude/worktrees/`), never by raw-editing the primary checkout. Two hooks in `.claude/settings.json` keep this true: `session-start` orients you, and `guard-worktree` (PreToolUse on Edit/Write/NotebookEdit) blocks edits in the primary checkout — only `.claude/` config stays editable so you're never locked out. Never `git switch`/`checkout` a branch in the primary checkout; if you find it parked off `main`, switch it back.
- **Everything past isolation is judgment, not choreography.** When the work is real, push the branch and open a normal PR (`gh pr create --fill`) — no PR-at-start, no drafts. If a session produced nothing worth shipping, exit the worktree and let it go.
- When the work is **genuinely done and clean**, merge it yourself with plain `gh` — `gh pr merge --squash --delete-branch` — then `ExitWorktree` (remove). Don't ask the user to merge, and don't merge over doubt: hold off if the work is unfinished, a check is red (fix it first), or you were told to wait. After an agent merge, **echo 🚀 in your reply** — a chat/session signal only; it does not go in the PR title or anywhere on GitHub.
...
- `Games/` — the only place games live, one directory per game (`@games/<name>`, private, source-consumed via `./src` exports, no build). Games are built via the `harvest-game` skill, which builds a minimal playable slice and files the engine gaps it exposes as `[FEATURE]` issue(s) at session end.
