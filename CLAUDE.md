...
- **Issues are written for a cold reader — structure is not optional.** Every issue this repo files gets markdown structure: `## Problem` / `## Context` / `## Suggested scope` headings, never a wall of prose. The first sentence under **Problem** states plainly what's missing or broken and why it matters — no burying the ask, no beating around the bush. **Context** carries what a reader without your session needs: what exists today, links to relevant code and issues. **Suggested scope** stays at the primitive level — describe the gap, don't over-prescribe one implementation. An issue that opens with implementation trivia before stating the problem (see the pattern that produced [#442](https://github.com/Noisemaker111/jgengine/issues/442)) fails this bar.
- **Engine gaps and improvement ideas are never papercuts.** Hitting one mid-task: note it in your working notes and keep going — don't stop to file, don't `bun run papercut` it. At session end (or just before starting a big implementation), file the accumulated notes as proper issues in one pass, each meeting the issue bar above.
