...
function ruskShape(body: string, accent: string): string {
  return `
...
function shapeFor(def: EnemyDef, colors: { body: string; accent: string }): string {
  ...
  if (def.id === "captain_rusk" || def.id === "bad_maw") return ruskShape(colors.body, colors.accent);
...
export const entitySprites: Record<string, EntitySpriteConfig> = {
  ...
  bolt: { url: billboard(boltShape()), width: 1.1, height: 1.4, y: 0.7 },
};
