...
export function BoltRig({ entity }: { entity: SceneEntity }) {
  const { root, phase } = useGait(entity.id);
...
const NPC_STYLES: Record<string, { coat: string; skin: string; hat: string }> = {
  dr_sparx: { coat: "#d8d4c8", skin: "#c99a72", hat: "#e8e4da" },
  rigg: { coat: "#8a6a1e", skin: "#b8865a", hat: "#4a3a2c" },
  gauge: { coat: "#3a4a5e", skin: "#8a6a4a", hat: "#26303c" },
};

export function NpcRig({ entity }: { entity: SceneEntity }) {
  const { root, phase } = useGait(entity.id);
  const torso = useRef<Group>(null);
  const style = NPC_STYLES[entity.name] ?? NPC_STYLES.dr_sparx!;
  useFrame((state) => {
...
export function renderFerralonEntity(entity: SceneEntity): ReactNode {
  if (entity.name === "bolt") return <BoltRig entity={entity} />;
