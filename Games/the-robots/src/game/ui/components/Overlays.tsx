...
import { ffylStore, ruskDownStore, vendorOpenStore } from "../../stores";
  ...
  return (
    <div className="pointer-events-none absolute inset-0 z-40 flex flex-col items-center justify-center">
      ...
      <span className="mt-2 text-lg font-bold text-stone-100">
        Get a kill for a Power Surge — {secondsLeft.toFixed(1)}s
      </span>
    </div>
  );
}
...
function BuyRow({ label, detail, cost, affordable, onBuy }: { label: string; detail?: string; cost: number; affordable: boolean; onBuy: () => void }) {
  return (
    ...
    >
      ...
      <span className="text-sm font-black tabular-nums" style={{ color: FERRALON.hudXp }}>
        ${cost}
      </span>
    </button>
  );
...
export function VendorPanel() {
  ...
  const ruskDown = useStore(ruskDownStore);
  ...
  if (vendor === "bolt") {
    return (
      <PanelFrame title="B0-LT Steward Bot" onClose={close}>
        <p className="text-sm font-semibold leading-relaxed text-stone-200">
          {ruskDown
            ? "OH MY GOD YOU ACTUALLY DID IT. Captain Rusk is dead and I am ALIVE. Sanctuary awaits, minion! (This demake ends here — go hunt legendaries.)"
            : "Minion! Scrapjacks took over the camp east of here, the ripper drones are chewing my spare parts, and a rusted-out unit called Captain Rusk keeps threatening to melt me down. Check your mission log — and try not to die, it looks painful."}
        </p>
  ...
  if (vendor === "sparx") {
    return (
      <PanelFrame title="Dr. Sparx's Meds" onClose={close}>
  ...
  return (
    <PanelFrame title="Rigg Munitions" onClose={close}>
      <div className="flex flex-col gap-1.5">
        {AMMO_POOLS.map((pool) => (
          <BuyRow
            key={pool}
        ...
        <BuyRow
          ...
          detail="A mystery roll from Rigg's back room"
          cost={300}
          affordable={cash >= 300}
          onBuy={() => commands.run("vendor.gunOfTheDay", { vendor: "rigg" })}
        />
      </div>
