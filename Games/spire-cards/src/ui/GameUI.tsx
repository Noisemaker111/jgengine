...
import { CardArtIcon, IntentIcon } from "./icons";
...
function intentLabel(intent: Intent): string {
  if (intent.kind === "attack") return `Attacks for ${intent.value}`;
  if (intent.kind === "defend") return `Guards ${intent.value} Block`;
  return `Buffs +${intent.value} Str`;
}
    ...
    <div className="absolute left-1/2 top-10 flex -translate-x-1/2 flex-col items-center gap-3">
      {intent && (
        <div className="flex items-center gap-2 rounded-lg border border-amber-400/50 bg-stone-950/80 px-3 py-1.5 shadow-lg shadow-black/40">
          <IntentIcon
            kind={intent.kind}
            className={intent.kind === "attack" ? "h-5 w-5 text-rose-300" : intent.kind === "defend" ? "h-5 w-5 text-sky-300" : "h-5 w-5 text-amber-300"}
          />
...
function ResultOverlay({ phase, onRestart }: { phase: Phase; onRestart: () => void }) {
  if (phase !== "won" && phase !== "lost") return null;
  const won = phase === "won";
  const restartKey = actionLabel(keybinds, "restartCombat");
  return (
...
export function GameUI() {
  ...
  const restart = () => commands.run("restartCombat", {});
  const playerTurn = snapshot.phase === "player";
  ...
  return (
    <div className="relative h-full w-full select-none overflow-hidden text-stone-100">
      ...
      <ResultOverlay phase={snapshot.phase} onRestart={restart} />
    </div>
  );
}
