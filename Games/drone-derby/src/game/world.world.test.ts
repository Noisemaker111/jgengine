...
describe("drone-derby world", () => {
  ...
  test("declares terrain with finite relief", () => {
  ...
  test("has the expected structure groups and total building count", () => {
    expect(summary.counts.structureGroups).toBe(6);
    expect(summary.counts.buildings).toBe(50);
  });
});
