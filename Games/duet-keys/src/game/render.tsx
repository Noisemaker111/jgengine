...
function resolveModel(id: string): ModelAssetRef | null {
  return assets.resolve(id);
}

function mustResolve(id: string): ModelAssetRef {
  const entry = resolveModel(id);
  if (entry === null) throw new Error(`duet-keys: unknown model id "${id}"`);
  return entry;
}

function tinted(id: string, material: ModelMaterialOverride): ModelConfig | null {
  const entry = resolveModel(id);
...
for (const id of Object.values(DUNGEON)) {
  const entry = resolveModel(id);
  if (entry !== null) useLoader.preload(gltfLoader, entry.url);
}

/** Wall/emitter/exit — static objectModels when catalog is live. */
export const objectModels: Record<string, string | ModelConfig> = {};
{
  const wall = resolveModel(DUNGEON.wall);
  if (wall !== null) objectModels.wall = DUNGEON.wall;
  const emitter = tinted(DUNGEON.rocks, {
    color: objectStyles.emitter.color,
    emissive: "#bafcff",
    emissiveIntensity: 0.55,
  });
  if (emitter !== null) objectModels.emitter = emitter;
  const exitL = tinted(DUNGEON.stairs, {
    color: HEROES.lumen.color,
    emissive: HEROES.lumen.glow,
    emissiveIntensity: 0.45,
  });
  if (exitL !== null) objectModels.exit_lumen = exitL;
  const exitA = tinted(DUNGEON.stairs, {
    color: HEROES.anchor.color,
    emissive: HEROES.anchor.glow,
    emissiveIntensity: 0.45,
  });
  if (exitA !== null) objectModels.exit_anchor = exitA;
}

const TINTED_OBJECT_MODELS: Partial<Record<ObjectId, string>> = {
  gate: DUNGEON.gate,
...
function HeroRing({ glow, isActive }: { glow: string; isActive: boolean }) {
  return (
    <mesh rotation-x={-Math.PI / 2} position-y={0.05}>
      <ringGeometry args={[0.5, 0.62, 24]} />
      <meshBasicMaterial color={glow} transparent opacity={isActive ? 0.95 : 0.25} side={THREE.DoubleSide} />
    </mesh>
  );
}

function HeroPrimitive({ hero, isActive }: { hero: (typeof HEROES)[keyof typeof HEROES]; isActive: boolean }) {
  return (
    <group>
      ...
      <HeroRing glow={hero.glow} isActive={isActive} />
    </group>
  );
}
...
function HeroGlb({
  ...
  modelId: string;
  hero: (typeof HEROES)[keyof typeof HEROES];
  isActive: boolean;
}) {
  ...
  return (
    <group>
      <primitive object={scene} position={position} scale={heroScale} />
      <HeroRing glow={hero.glow} isActive={isActive} />
    </group>
  );
}
...
function HeroMesh({ entity }: { entity: SceneEntity }) {
  ...
  if (resolveModel(modelId) === null) return <HeroPrimitive hero={hero} isActive={isActive} />;
  return <HeroGlb modelId={modelId} hero={hero} isActive={isActive} />;
}
...
function DungeonObjectMesh({ object }: { object: SceneObject }) {
  ...
  if (resolveModel(modelId) === null) return null;
...
export function renderDuetObject(object: SceneObject): ReactNode {
  if (!(object.catalogId in TINTED_OBJECT_MODELS)) return null;
  return (
    <ModelBoundary>
...
function FloorTiles({ room }: { room: RoomDef }) {
  const urls = FLOOR_VARIANT_IDS.map((id) => resolveModel(id)?.url).filter((u): u is string => u !== undefined);
  if (urls.length === 0) {
    return (
      <mesh rotation-x={-Math.PI / 2} position-y={0} receiveShadow>
        <planeGeometry args={[20, 20]} />
        <meshStandardMaterial color={ROOM_BASE} roughness={0.9} />
      </mesh>
    );
  }
  const floor = useLoader(gltfLoader, urls[0]!);
  const detail = useLoader(gltfLoader, urls[Math.min(1, urls.length - 1)]!);
  const dirt = useLoader(gltfLoader, urls[Math.min(2, urls.length - 1)]!);
...
function PrismProp({ position }: { position: [number, number, number] }) {
  const modelId = resolveModel(DUNGEON.coin) !== null ? DUNGEON.coin : DUNGEON.torch;
  if (resolveModel(modelId) === null) return null;
  const { scene, entry } = useDungeonScene(modelId, {
  ...
  const propScale = 2.4;
  const local = centeredPosition(entry, 0.55, propScale);
  return (
    <group ref={groupRef} position={position}>
      <primitive object={scene} position={local} scale={propScale} />
    </group>
  );
}

function AnchorWeightProp({ position }: { position: [number, number, number] }) {
  if (resolveModel(DUNGEON.stones) === null) return null;
  const { scene, entry } = useDungeonScene(DUNGEON.stones, {
    color: "#2a2f3d",
    metalness: 0.5,
    roughness: 0.5,
  });
  const weightScale = 1.35;
  const local = centeredPosition(entry, 0, weightScale);
  return (
    <group position={position}>
      <primitive object={scene} position={local} scale={weightScale} />
    </group>
  );
}
