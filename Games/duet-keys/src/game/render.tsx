 ...
 * `useLoader(GLTFLoader, url, ...)` constructs. The shared default manager is process-wide
 * singleton state — under this dev server's repeated navigations it can end up with an
 * already-aborted internal `AbortController`, which `FileLoader.load()` composes into every future
 * request's abort signal, silently stalling `GLTFLoader.load()` forever with no thrown error
 * (parsing already-fetched bytes still works fine — it's a fetch-stage-only hang). A private
 * manager sidesteps the shared state; see the matching fix in `packages/shell/GamePlayerShell.tsx`.
 */
...
function tinted(id: string, material: ModelMaterialOverride): ModelConfig {
...
export const objectModels: Record<string, string | ModelConfig> = {
  ...
  exit_lumen: tinted(DUNGEON.stairs, {
    color: HEROES.lumen.color,
    emissive: HEROES.lumen.glow,
    emissiveIntensity: 0.45,
  }),
  exit_anchor: tinted(DUNGEON.stairs, {
...
/** Objects whose material/position react to live room state — rendered through `renderObject`
 * instead of the static `objectModels` map so `object.visual` drives the tint each frame. */
...
/** The two heroes, rendered as real dungeon-kit characters — cyan-tinted human (Lumen) and
 * orange-tinted orc (Anchor) — with a floor ring marking which one the player is driving. */
