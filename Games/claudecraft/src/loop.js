...
const EMPTY_INPUT = { held: [], pointer: null };
function tickPlayerSystems(ctx, userId, dt) {
...
function onPlayerDied(ctx, userId, position) {
...
function onKill(ctx, killerUserId, victimInstanceId) {
    ...
    if (restedBonus > 0)
        restedStore.write(ctx, userId, pool - restedBonus);
...
export const loop = {
    onInit(ctx) {
        ...
        for (const table of buildLootTables())
            ctx.game.loot.register(table);
        ...
        ctx.game.events.on("entity.died", (evt) => {
            ...
            if (evt.reason.kind === "player_kill")
                onKill(ctx, killerUserId, evt.instanceId);
            else
                onMobDied(ctx, evt.instanceId);
            if (wasWorldBoss)
                onWorldBossKilled(ctx, evt.instanceId, killerUserId);
        });
        setupWorld(ctx);
    },
