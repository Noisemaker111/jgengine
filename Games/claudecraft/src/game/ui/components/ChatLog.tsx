...
const CHANNELS = ["global", "party", "proximity"] as const;
...
export function ChatLog() {
  return (
    <ChatPanel
      channels={CHANNELS}
      className="wcc-panel flex w-[370px] flex-col overflow-hidden text-xs"
      ...
      activeTabClassName="wcc-title !text-[#ffd100]"
      ...
      sendButtonClassName={BUTTON}
      placeholder="Say something..."
      renderTab={(channelId, isActive) => (
        <span className={isActive ? undefined : "opacity-80"}>{CHANNEL_LABELS[channelId] ?? channelId}</span>
      )}
      renderMessage={(message: ChatMessage) => (
        <>
          <span className="font-semibold text-[#9fdc7f]">{message.fromUserId}: </span>
          <span className="text-stone-200">{message.body}</span>
        </>
      )}
    />
  );
