...
function ClockReadout() {
    ...
    return (_jsxs("span", { children: [padNumber(calendar.hour, 2), ":", padNumber(calendar.minute, 2)] }));
}
const QUESTGIVER_IDS = new Set(NPCS.filter((npc) => npc.kind === "questgiver").map((npc) => `npc:${npc.id}`));
export function Minimap() {
    ...
    useEffect(() => {
        ...
        for (const entity of ctx.scene.entity.list()) {
            if (entity.id === ctx.player.userId)
                continue;
        ...
        for (const item of ctx.scene.worldItem.list()) {
            ...
            if (position !== undefined)
                markers.add({ kind: "loot", position });
        }
    }, [tick, ctx, markers]);
    ...
    if (player === null)
        return null;
    return (_jsx(MinimapPanel, { markers: markers, center: [player.position[0], player.position[2]], worldRadius: 46, size: 168, facingYaw: player.rotationY, rotate: true, title: zoneName ?? "Map", clock: _jsx(ClockReadout, {}), compassProps: { width: 168 } }));
}
