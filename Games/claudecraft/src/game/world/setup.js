...
export function dungeonPrompts(ctx) {
    void ctx;
    const prompts = [];
...
export function strongboxPrompts(ctx) {
...
export function contentPrompts(ctx) {
    void ctx;
    const prompts = [];
    for (const zone of ZONES) {
        prompts.push({
...
export function npcPrompts(ctx) {
    const prompts = [];
    for (const npc of NPCS) {
        const entity = ctx.scene.entity.get(`npc:${npc.id}`);
        if (entity === null || npc.dialogueId === undefined)
            continue;
        prompts.push({
            id: `talk:${npc.id}`,
...
export function graveyardOf(x, z) {
    let best = [ZONES[0].graveyard.x, ZONES[0].graveyard.z];
