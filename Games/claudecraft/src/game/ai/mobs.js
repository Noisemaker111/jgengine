import { createMobBrain } from "@jgengine/core/ai/mobBrain";
...
const runtimesOf = perContext(() => new Map());
...
function placementFor(def, roll, index) {
    ...
    for (let attempt = 0; attempt < 40; attempt += 1) {
        ...
        if (hubDist > hub.radius + ZONE_EDGE_MARGIN && graveDist > ZONE_EDGE_MARGIN && cryptDist > CRYPT.radius + 8)
            return [x, z];
    }
    return [zone.hub.x + 40 + index * 3, (zone.zMin + zone.zMax) / 2];
}
export function spawnMobAt(ctx, def, position, level, options) {
    ...
    const spawn = [position[0], y, position[1]];
...
function spawnMob(ctx, def, position, level) {
    return spawnMobAt(ctx, def, position, level);
}
export function applyMobCc(ctx, instanceId, sourceId, cc) {
    const runtime = runtimesOf(ctx).get(instanceId);
    if (runtime === undefined || runtime.brain.mode() === "evade")
        return false;
...
export function spawnAllMobs(ctx) {
    let spawned = 0;
...
export function onMobDied(ctx, instanceId) {
    const runtime = runtimesOf(ctx).get(instanceId);
    if (runtime === undefined)
        return null;
    ...
    if (def !== null && def.rare !== true && def.boss !== true && runtime.noRespawn !== true) {
        const respawnAt = [runtime.spawn[0], runtime.spawn[2]];
...
export function despawnMob(ctx, instanceId) {
...
function socialPull(ctx, def, instanceId, targetId) {
    ...
    for (const allyId of ctx.scene.entity.inRadius(self.position, radius, (id) => isMobInstance(ctx, id))) {
        if (allyId === instanceId)
            continue;
        const ally = runtimesOf(ctx).get(allyId);
        if (ally === undefined || ally.defId !== def.id || ally.brain.mode() === "evade")
            continue;
        if (ally.brain.threat.highest() === null)
            ally.brain.addThreat(targetId, 1);
        if (def.packFrenzy !== undefined)
            ally.frenzyUntil = ctx.time.now() + def.packFrenzy.duration;
    }
}
function swingAtPlayer(ctx, def, runtime, instanceId, targetId) {
    const now = ctx.time.now();
    if (now < runtime.nextSwingAt)
        return;
    const sheet = heroSheet(ctx, targetId);
    const weakenPct = aurasOf(ctx, instanceId).reduce((sum, aura) => (aura.buffStat === "attackPower" && (aura.buffAmount ?? 0) < 0 ? sum + (aura.buffAmount ?? 0) : sum), 0);
    const enrage = runtime.enraged ? def.mechanics?.enrage : undefined;
    const raw = Math.max(1, Math.round(mobDamage(def.dmgBase, def.dmgPerLevel, runtime.level) *
    ...
    const haste = (now < runtime.frenzyUntil ? (def.packFrenzy?.hasteMult ?? 1) : 1) * (enrage?.hasteMult ?? 1);
    runtime.nextSwingAt = now + def.attackSpeed / haste;
}
function castMobAbility(ctx, def, runtime, instanceId) {
    ...
    for (const ability of abilities) {
        ...
        if (now < nextAt)
            continue;
        runtime.abilityAt.set(ability.id, now + ability.intervalSec);
        const at = [self.position[0], self.position[1], self.position[2]];
...
function runBossMechanics(ctx, def, runtime, instanceId, targetId, now) {
    ...
    if (summons !== undefined) {
        ...
        if (now >= runtime.nextSummonAt) {
            ...
            const self = ctx.scene.entity.get(instanceId);
            if (addDef === null || self === null || room <= 0)
                return;
            for (let index = 0; index < Math.min(room, summons.count); index += 1) {
                const angle = (index / summons.count) * Math.PI * 2;
                const addId = spawnMob(ctx, addDef, [self.position[0] + Math.cos(angle) * 3, self.position[2] + Math.sin(angle) * 3], Math.max(addDef.minLevel, runtime.level - 1));
...
export function tickMobs(ctx, dt) {
    ...
    for (const [instanceId, runtime] of runtimesOf(ctx)) {
        ...
        if (brain.mode() !== "evade") {
            ...
            if (brain.threat.highest({ stickiness: 1.15 }) === null && player !== null && !playerDead) {
                ...
                if (playerDist <= aggroRadius) {
                    ...
                    if (def.packFrenzy !== undefined)
                        runtime.frenzyUntil = now + def.packFrenzy.duration;
                }
            }
        }
        ...
        if (step.targetId !== null && now < runtime.stunnedUntil)
            continue;
        if (step.moveTo !== null) {
        ...
        if (step.targetId !== null) {
            if (def.abilities !== undefined)
                castMobAbility(ctx, def, runtime, instanceId);
            if (def.mechanics !== undefined)
                runBossMechanics(ctx, def, runtime, instanceId, step.targetId, now);
        }
    }
}
export function mobCount(ctx) {
    return runtimesOf(ctx).size;
}
export function resetMobs(ctx) {
