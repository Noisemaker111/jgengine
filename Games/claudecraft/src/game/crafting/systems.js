...
export const FISHING_CHECK = {
    trackWidth: 1,
    zone: { start: 0.38, end: 0.62 },
    markerPeriod: 1.6,
    window: 6,
};
const fishingSessionsOf = perContext(() => new Map());
const fishRoll = seededRng("claudecraft-fishing");
export function placeCraftingWorld(ctx) {
...
export function craftingPrompts(ctx) {
    void ctx;
    const prompts = ZONES.map((zone) => ({
        id: `forge:${zone.id}`,
...
export function craftRecipe(ctx, userId, recipeId) {
    const recipe = RECIPES.find((entry) => entry.id === recipeId);
    if (recipe === undefined)
        return;
    const say = (text) => ctx.scene.entity.floatText({ instanceId: userId, text, kind: "info" });
    ...
    for (const input of recipe.inputs)
        ctx.player.inventory.take("bags", input.itemId, input.count);
...
export function castFishing(ctx, userId) {
