...
import { CRIT_MULTIPLIER, GCD_SEC, MELEE_RANGE, mitigate, rollCrit, rollWeaponDamage, spellAmount, } from "../math/combat";
import { abilityModsOf, auraEntries, aurasOf, barOf, classOf, enterCombat, externalCombatModsOf, gainRage, heroOf, heroSheet, syncAuras, } from "../session/hero";
...
import { consumeNextCastFree, fireSpellCastProcs, fireWeaponCritProcs, } from "./setProcs";
...
export function targetOf(ctx, userId) {
...
function hostileTarget(ctx, userId) {
    const targetId = targetOf(ctx, userId);
    if (targetId === null)
        return null;
    return isMobInstance(ctx, targetId) ? targetId : null;
}
function applyAura(ctx, targetId, sourceId, ability, amount, buff) {
...
function buffFlatAmount(sheet, ability) {
    if (ability.buffStat === undefined)
        return {};
    const pct = ability.buffAmount ?? ability.base;
    const basis = ability.buffStat === "attackPower"
        ? sheet.attackPower
        : ability.buffStat === "spellPower"
            ? Math.max(sheet.spellPower, 20)
            : ability.buffStat === "armor"
                ? sheet.armor
                : sheet.attributes[ability.buffStat];
    return { stat: ability.buffStat, amount: Math.max(1, Math.round((basis * pct) / 100)) };
}
function dealDamage(ctx, userId, targetId, rawAmount, crit, physical = true, school) {
...
function abilityAmount(ctx, userId, ability, sheet) {
    ...
    if (ability.school === "physical") {
        ...
        amount = spellAmount(ability.base, ability.perLevel, level, sheet.spellPower, ability.coefficient, castTime);
...
function applyCc(ctx, userId, targetId, ability) {
    const cc = ability.cc;
    if (cc === undefined)
        return;
    if (cc.kind === "armorShred") {
        applyAura(ctx, targetId, userId, { ...ability, kind: "buff", duration: cc.durationSec }, 0, { stat: "armor", amount: -Math.abs(cc.amount ?? ability.base) });
...
function executeAbility(ctx, userId, ability) {
    ...
    if (ability.cc !== undefined) {
        ...
        if (ccTarget !== null)
            applyCc(ctx, userId, ccTarget, ability);
        if (ability.kind === "buff" && ability.base === 0)
            return;
    }
    ...
    switch (ability.kind) {
        case "damage": {
            ...
            dealDamage(ctx, userId, targetId, abilityAmount(ctx, userId, ability, sheet), crit, ability.school === "physical", ability.school);
            if (crit && ability.school === "physical")
                fireWeaponCritProcs(ctx, userId, sheet, targetId);
        ...
        case "buff": {
            if (ability.selfTarget === false) {
                ...
                if (targetId === null)
                    return;
                const targets = ability.aoeRadius !== undefined
                    ? ctx.scene.entity.inRadius(ctx.scene.entity.get(targetId)?.position ?? [0, 0, 0], ability.aoeRadius, (id) => isMobInstance(ctx, id))
                    : [targetId];
...
export function applyFood(ctx, userId, item) {
    const now = ctx.time.now();
    ...
    for (const [auraId, kind] of [
        ...
        if (amountBase === undefined || amountBase <= 0)
            continue;
...
export function castSlot(ctx, userId, slot) {
    const cls = classOf(ctx, userId);
    const hero = heroOf(ctx, userId);
    if (cls === null || hero === null)
        return;
    if (deadStore.read(ctx, userId))
        return;
    const level = ctx.scene.entity.stats.get(userId, "level")?.current ?? 1;
    ...
    if (snapshot === null || !snapshot.ready) {
        if (snapshot?.state === "no-resource")
            say(ctx, userId, "Not enough " + cls.resource);
        return;
    }
...
function commitCast(ctx, userId, ability) {
    const hero = heroOf(ctx, userId);
    if (hero === null)
        return;
    const resource = ctx.scene.entity.stats.get(userId, "resource")?.current ?? 0;
    ...
    if (tunedCost > 0)
        ctx.scene.entity.stats.delta(userId, "resource", -tunedCost);
    ...
    if (isSpell) {
        const sheet = heroSheet(ctx, userId);
        if (sheet !== null)
            fireSpellCastProcs(ctx, userId, sheet);
    }
}
export function tickHero(ctx, userId, dt) {
    const hero = heroOf(ctx, userId);
    const cls = classOf(ctx, userId);
    if (hero === null || cls === null)
        return;
    hero.kit.tick(dt);
    const now = ctx.time.now();
    ...
    if (now >= hero.regenAt) {
        hero.regenAt = now + 2;
        const inHub = ZONES.some((zone) => Math.hypot(self.position[0] - zone.hub.x, self.position[2] - zone.hub.z) <=
            zone.hub.radius + 6);
        if (inHub) {
        ...
        if (sheet !== null && !deadStore.read(ctx, userId)) {
            ...
            if (!fighting)
                stats.delta(userId, "health", Math.round(sheet.attributes.spi * 0.3) + 1);
        }
    }
}
export function tickAuras(ctx) {
    const now = ctx.time.now();
