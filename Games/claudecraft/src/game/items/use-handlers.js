...
export const useHandlers = {
    consume: {
        can(ctx, input) {
            ...
            if (item === null)
                return { reason: "unknown-item" };
            if (!COMBAT_SAFE.test(item.id) && inCombat(ctx, input.from)) {
                return { reason: "Cannot eat or drink in combat" };
            }
            if (ctx.player.inventory.count("bags", input.itemId) < 1)
                return { reason: "none-left" };
            return null;
        },
        apply(ctx, input) {
            const item = itemDefById(input.itemId);
            if (item === null)
                return { state: ctx, error: "unknown-item" };
    ...
    equip: {
        can(ctx, input) {
            const item = itemDefById(input.itemId);
            if (item === null || item.slot === undefined)
                return { reason: "not-equippable" };
        ...
        apply(ctx, input) {
            const item = itemDefById(input.itemId);
            if (item === null || item.slot === undefined)
                return { state: ctx, error: "not-equippable" };
            const slot = item.slot;
            const equips = { ...equipsOf(ctx, input.from) };
            const previous = equips[slot];
            ctx.player.inventory.take("bags", input.itemId, 1);
            if (previous !== undefined)
                ctx.player.inventory.put("bags", previous, 1);
