...
export function isDisenchantable(def) {
    return def !== null && (def.kind === "weapon" || def.kind === "armor") && def.quality !== "poor";
}
export function disenchantYield(def, roll) {
...
export function aggregateEnchantBonuses(equips, enchants) {
    const out = {
        str: 0,
        agi: 0,
        sta: 0,
    ...
    for (const [slot, enchantId] of Object.entries(enchants)) {
        if (enchantId === undefined || equips[slot] === undefined)
            continue;
        const enchant = enchantById(enchantId);
        if (enchant === null || enchant.itemSlot !== slot)
            continue;
