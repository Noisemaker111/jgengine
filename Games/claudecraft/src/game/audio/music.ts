...
const n = (beat: number, midi: number, dur: number, vel: number, inst: MusicInstrument): NoteEvent => ({
  beat,
  midi,
  dur,
  vel,
...
function chord(beat: number, midis: readonly number[], dur: number, vel: number, inst: MusicInstrument): NoteEvent[] {
  return midis.map((m) => n(beat, m, dur, vel, inst));
}

function arp(start: number, midis: readonly number[], reps: number, step: number, dur: number, vel: number, inst: MusicInstrument): NoteEvent[] {
  const out: NoteEvent[] = [];
  for (let i = 0; i < reps; i += 1) out.push(n(start + i * step, midis[i % midis.length]!, dur, vel, inst));
  return out;
}

function pulse(start: number, midi: number, reps: number, step: number, dur: number, vel: number, inst: MusicInstrument): NoteEvent[] {
  const out: NoteEvent[] = [];
  for (let i = 0; i < reps; i += 1) out.push(n(start + i * step, midi, dur, vel, inst));
  return out;
}
