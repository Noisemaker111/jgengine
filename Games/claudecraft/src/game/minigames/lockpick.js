import { generateLock, LOCK_ACTIONS, stepLock, visibleCells, } from "@jgengine/core/interaction/lockpick";
...
export function lockboxLockedOut(ctx, instanceId, userId) {
    const expiry = ctx.game.store.get(lockoutKey(instanceId, userId)) ?? 0;
    return ctx.time.now() < expiry;
}
export function lockboxPrompts(ctx) {
...
export function engageLockpick(ctx, userId, instanceId, ante = 2) {
    if (sessions.has(userId))
        return false;
    if (!boxes.has(instanceId))
        return false;
    if (lockboxLockedOut(ctx, instanceId, userId)) {
...
export function pickLock(ctx, userId, action) {
    const session = sessions.get(userId);
    if (session === undefined || session.result !== "playing")
        return false;
    if (!session.spec.tier.allowedActions.includes(action))
        return false;
...
export function closeLockpick(ctx, userId) {
    const had = sessions.delete(userId);
    ctx.game.store.delete(viewKey(userId));
    return had;
}
export function lockpickActive(userId) {
...
function sync(ctx, userId) {
    ...
    if (session.result !== "playing")
        sessions.delete(userId);
}
export function resetLockboxes() {
    sessions.clear();
    boxes.clear();
}
export function lockboxCount() {
    return boxes.size;
}
