...
export const YUMI_ENTRANCE = sceneMarkerXZ("landmark:yumi_entrance");
export const YUMI_ARENA = sceneMarkerXZ("landmark:yumi_arena");
...
export function leaveProtectYumi(ctx, userId) {
...
function cleanup(ctx, session) {
    for (const id of session.spawned)
        despawnMob(ctx, id);
    session.spawned = [];
    if (ctx.scene.entity.get(session.yumiId) !== null)
        ctx.scene.entity.despawn(session.yumiId);
}
export function tickProtectYumi(ctx, userId, dt) {
    const session = sessionsOf(ctx).get(userId);
    if (session === undefined || session.status !== "playing")
        return;
...
export function yumiActive(ctx, userId) {
