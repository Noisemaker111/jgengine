...
export function mitigate(amount, armor, attackerLevel) {
    return Math.max(1, Math.round(amount * (1 - armorReduction(armor, attackerLevel))));
}
export function rollWeaponDamage(rng, weapon, attackPower) {
    const base = weapon.min + rng() * (weapon.max - weapon.min);
    return Math.max(1, Math.round(base + (attackPower / 14) * weapon.speed));
}
export function spellAmount(base, perLevel, level, spellPower, coefficient, castTime) {
...
export function rollCrit(rng, critPct) {
    return rng() * 100 < critPct;
}
export function fallDamage(maxHp, dropDistance) {
    if (dropDistance <= FALL_SAFE_DISTANCE)
        return 0;
    return Math.round(maxHp * (dropDistance - FALL_SAFE_DISTANCE) * 0.07);
}
export function mobHp(hpBase, hpPerLevel, level) {
    return Math.round(hpBase + hpPerLevel * level);
}
export function mobDamage(dmgBase, dmgPerLevel, level) {
