...
import {
  ...
  FIESTA_COUNTDOWN,
  FIESTA_ENEMY_MOBS,
  FIESTA_FIRST_WAVE_AT,
  FIESTA_MAX_DURATION,
  FIESTA_POWERUP_FIRST,
  FIESTA_POWERUP_INTERVAL,
  FIESTA_POWERUP_MAX,
  FIESTA_POWERUP_OBJECT,
  FIESTA_POWERUP_RADIUS,
...
type Team = "a" | "b";
type FighterKey = "player" | "ally" | "enemy1" | "enemy2";
...
interface FiestaSession {
  ...
  offer: string[] | null;
  augments: string[];
  playerPowerups: { def: PowerupDef; expiresAt: number }[];
  fighters: Fighter[];
  returnPos: readonly [number, number];
  levelSnapshot: { level: number; xp: number; xpMax: number };
  arenaObjects: string[];
  firstBlood: boolean;
  playerStreak: number;
  playerLastKillAt: number;
  overAt: number | null;
  result: "victory" | "defeat" | "draw" | null;
  pop: WordPop | null;
  nextAllyHealAt: number;
  nextAllySwingAt: number;
  roll: () => number;
}
...
export interface FiestaView {
  ...
  ringRadius: number;
  inRing: boolean;
  fighters: FiestaFighterView[];
  offer: { id: string; name: string; description: string; tier: string }[] | null;
...
export interface FiestaRecord {
  wins: number;
  losses: number;
}

export function fiestaStoreKey(userId: string): string {
  return `fiesta:${userId}`;
}

export function fiestaRecordKey(userId: string): string {
...
function placeArena(ctx: GameContext, session: FiestaSession): void {
  ...
  const groundAt = (x: number, z: number) => ctx.world.groundHeightAt(x, z);
  const daisY = groundAt(cx + ARENA_DAIS[0], cz + ARENA_DAIS[1]);
  session.arenaObjects.push(
    ctx.scene.object.place(ARENA_DAIS_OBJECT, cx + ARENA_DAIS[0], daisY, cz + ARENA_DAIS[1], {
  ...
  for (const [px, pz] of ARENA_PILLARS) {
    ...
    const z = cz + pz;
    session.arenaObjects.push(
      ctx.scene.object.place(ARENA_PILLAR_OBJECT, x, groundAt(x, z) + 2, z, {
  ...
  for (const [sx, sz] of ARENA_STUBS) {
    ...
    const z = cz + sz;
    session.arenaObjects.push(
      ctx.scene.object.place(ARENA_PILLAR_OBJECT, x, groundAt(x, z) + 0.6, z, {
        visual: { scale: [1.2, 1.2, 8], color: "#5c5348" },
      }),
    );
  }
  const walls: readonly (readonly [number, number, readonly [number, number, number]])[] = [
  ...
  for (const [x, z, scale] of walls) {
    session.arenaObjects.push(
      ctx.scene.object.place(ARENA_WALL_OBJECT, x, groundAt(x, z) + 2, z, {
...
function spawnEnemy(ctx: GameContext, session: FiestaSession, fighter: Fighter): void {
...
export function startFiesta(ctx: GameContext, userId: string): boolean {
  ...
  for (const fighter of session.fighters) {
    if (fighter.key === "ally") spawnAlly(ctx, session, fighter);
    else if (fighter.enemyDefId !== undefined) spawnEnemy(ctx, session, fighter);
  }
...
function applyAugments(ctx: GameContext, userId: string, session: FiestaSession): void {
  const mods: ExternalCombatMods = {
    meleeDmgPct: 0,
    spellDmgPct: 0,
    healPct: 0,
    maxHpPct: 0,
...
function updatePlayerSpeed(ctx: GameContext, userId: string, session: FiestaSession): void {
  let pct = 0;
  for (const id of session.augments) pct += augmentById(id)?.moveSpeedPct ?? 0;
  const now = ctx.time.now();
  for (const active of session.playerPowerups) {
    if (now < active.expiresAt) pct += active.def.moveSpeedPct ?? 0;
  }
...
function scoreFor(session: FiestaSession, team: Team): number {
  return team === "a" ? session.scoreA : session.scoreB;
}

function addScore(session: FiestaSession, team: Team, amount: number): void {
  if (team === "a") session.scoreA += amount;
...
function checkWin(ctx: GameContext, userId: string, session: FiestaSession): void {
...
function endMatch(ctx: GameContext, userId: string, session: FiestaSession): void {
  session.status = "over";
  ...
  pop(
    ...
    session,
    userId,
    session.result === "victory" ? "VICTORY!" : session.result === "defeat" ? "DEFEAT" : "DRAW",
    session.result === "victory" ? "#7fdc4f" : session.result === "defeat" ? "#ff7a6a" : "#d8cba0",
  );
...
function cleanupAndReturn(ctx: GameContext, userId: string, session: FiestaSession): void {
  for (const fighter of session.fighters) {
    if (fighter.key === "player" || fighter.entityId === null) continue;
    if (fighter.key === "ally") {
      if (ctx.scene.entity.get(fighter.entityId) !== null) ctx.scene.entity.despawn(fighter.entityId);
    } else {
...
function grabPowerup(ctx: GameContext, userId: string, session: FiestaSession, spawn: PowerupSpawn): void {
  const now = ctx.time.now();
...
function tickAlly(ctx: GameContext, userId: string, session: FiestaSession, dt: number): void {
  const ally = session.fighters[1];
  if (ally.entityId === null) return;
  ...
  for (const fighter of session.fighters) {
    if (fighter.team !== "b" || fighter.entityId === null) continue;
  ...
  if (target === null || target.entityId === null) return;
  if (best > 2.6) {
...
export function tickFiesta(ctx: GameContext, userId: string, dt: number): void {
  ...
  session.playerPowerups = session.playerPowerups.filter((entry) => now < entry.expiresAt);
  if (session.playerPowerups.length !== before) updatePlayerSpeed(ctx, userId, session);

  for (const fighter of session.fighters) {
    if (fighter.respawnAt === null || now < fighter.respawnAt) continue;
    if (fighter.key === "player") respawnPlayer(ctx, userId, session);
    else if (fighter.key === "ally") spawnAlly(ctx, session, fighter);
    else spawnEnemy(ctx, session, fighter);
  }
...
function sync(ctx: GameContext, userId: string): void {
  ...
  const now = ctx.time.now();
  const fighters: FiestaFighterView[] = session.fighters.map((fighter) => {
