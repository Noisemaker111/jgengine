...
export const ARENA_CENTER = sceneMarkerXZ("landmark:arena");
export const ARENA_SPAWN_A = [ARENA_CENTER[0], ARENA_CENTER[1] - 14];
export const ARENA_SPAWN_B = [ARENA_CENTER[0], ARENA_CENTER[1] + 18];
...
export const ARENA_STUBS = [
    [-11, 2],
    [11, 2],
];
export const ARENA_DAIS = [0, 2, 8];
...
export function eligibleAugments(tier, classId, owned) {
    return AUGMENTS.filter((aug) => {
        if (aug.tier !== tier || owned.includes(aug.id))
            return false;
        if (aug.casterOnly === true && !CASTER_CLASSES.has(classId))
            return false;
        if (aug.physicalOnly === true && CASTER_CLASSES.has(classId))
            return false;
        if (aug.healerOnly === true && !HEALER_CLASSES.has(classId))
            return false;
        return true;
    });
}
...
export const FIESTA_ENEMY_MOBS = [
    {
        id: "fiesta_bot_botzo",
...
export function fiestaEnemyById(id) {
    return FIESTA_ENEMY_MOBS.find((def) => def.id === id) ?? null;
}
export function fiestaRespawnTime(deaths, elapsed) {
    const raw = FIESTA_RESPAWN_BASE +
...
export function ringTargetForWave(wave) {
