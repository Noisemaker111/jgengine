...
const KAYKIT_CLIPS = { idle: "Idle", walk: "Walking_A", run: "Running_A", runSpeed: 6 } as const;
const CREATURE_RUN = { idle: "Idle", walk: "Walk", run: "Run", runSpeed: 6 } as const;
const CREATURE_GALLOP = { idle: "Idle", walk: "Walk", run: "Gallop", runSpeed: 6 } as const;

function humanoid(file: string, tint?: string): ModelConfig {
  return {
...
function skeleton(file: string, height = 2.5): ModelConfig {
  return {
...
function creature(
  file: string,
  height: number,
  clips: { idle: string; walk: string; run?: string; runSpeed?: number } | null = CREATURE_RUN,
): ModelConfig {
  return {
...
const CLASS_MODEL: Record<string, ModelConfig> = {
...
function mobModel(def: MobDef): ModelConfig {
  const id = def.id;
  if (/wolf|prowler/.test(id)) return creature("wolf_basic", 1.6, CREATURE_GALLOP);
  if (/greyjaw/.test(id)) return creature("greyjaw", 2.2, CREATURE_GALLOP);
  if (/boar/.test(id)) return creature("wild_boar", 1.45, CREATURE_GALLOP);
  if (/rat/.test(id)) return creature("fox", 0.7, CREATURE_GALLOP);
  if (/fox/.test(id)) return creature("fox", 1.0, CREATURE_GALLOP);
  if (/stag/.test(id)) return creature("stag", 1.9, CREATURE_GALLOP);
  if (/bear|cragmaw/.test(id)) return creature("yetialt", 2.2);
  if (/spider|widow|brood/.test(id))
    return creature("spider", def.boss === true ? 2 : 1.4, { idle: "Spider_Idle", walk: "Spider_Walk" });
  if (/murloc|mudfin|deepfen|bloat|mirejaw|sloomtooth/.test(id)) return creature("frog", 1.7);
  if (/troll|grubjaw/.test(id)) return creature("orc", 2.4);
  if (/ogre|drogmar|brutok/.test(id)) return creature("giant", 2.8, { idle: "Idle", walk: "Walk" });
  if (/kobold|sapper|foreman|tunnel|grix/.test(id))
    return creature("goblin", 2.1, { idle: "Idle", walk: "Walk" });
  if (/elemental|storm|shardlord/.test(id)) return creature("golelingevolved", 2.2, null);
  if (/ghost|drowned|wraith/.test(id)) return creature("ghost", 1.6, null);
  if (/shambler/.test(id)) return skeleton("skeleton_rogue");
  if (/morthen/.test(id)) return skeleton("skeleton_mage");
  if (/necromancer|maldrec/.test(id)) return skeleton("necromancer");
  ...
  if (def.family === "undead") return skeleton("skeleton_minion");
  if (def.family === "demon") return creature("demonalt", 2.1);
  if (def.family === "elemental") return creature("golelingevolved", 2.2, null);
  if (/gorrak|mogger/.test(id)) return humanoid("barbarian");
  if (def.family === "humanoid") return humanoid("rogue_hooded", "#8a5a4e");
  return creature("wolf_basic", 1.6, CREATURE_GALLOP);
}

function npcModel(def: NpcDef): ModelConfig {
  if (/marshal|warden|captain|guard/.test(def.id)) return humanoid("knight");
  if (/smith|armorer|foreman/.test(def.id)) return humanoid("barbarian");
  if (/scout/.test(def.id)) return humanoid("rogue");
  if (/apothecary|herbalist|loremaster|brother|sister|priest/.test(def.id))
    return humanoid("mage", "#c9b98a");
  if (def.kind === "vendor") return humanoid("rogue");
  return humanoid("rogue");
}

const PET_MODEL: Record<string, ModelConfig> = {
