...
export const GROUP_XP_BONUS = [1, 1, 1.166, 1.3, 1.43] as const;

export const levelTrack: LevelingTrack = leveling({
  xpForLevel: { kind: "steps", values: [XP_TABLE[0], ...XP_TABLE] },
  maxLevel: MAX_LEVEL,
});

function zeroDiff(playerLevel: number): number {
...
export function killXp(playerLevel: number, mobLevel: number): number {
...
export function groupXpMultiplier(partySize: number): number {
