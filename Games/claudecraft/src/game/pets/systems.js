...
import { defaultPetForClass, PET_ABILITY_IDS, petById, } from "./catalog";
const petsOf = perContext(() => new Map());
export function petViewOf(ctx, userId) {
    ...
    if (def === null)
        return null;
    const hp = runtime.instanceId === null
        ? { current: 0, max: 1 }
...
function petHp(def, level) {
    return Math.round(def.hpBase + def.hpPerLevel * (level - 1));
}
function spawnPetEntity(ctx, userId, def) {
...
export function summonPet(ctx, userId, petId) {
    const cls = classOf(ctx, userId);
    if (cls === null)
        return false;
    if (cls.id !== "hunter" && cls.id !== "warlock")
        return false;
    const level = ctx.scene.entity.stats.get(userId, "level")?.current ?? 1;
    const def = petId !== undefined
        ? petById(petId)
        : defaultPetForClass(cls.id);
    if (def === null || def.classId !== cls.id || def.levelReq > level)
        return false;
...
export function dismissPet(ctx, userId, announce = true) {
    const runtime = petsOf(ctx).get(userId);
    if (runtime === undefined)
        return false;
...
export function revivePet(ctx, userId) {
    const runtime = petsOf(ctx).get(userId);
    if (runtime === undefined || !runtime.dead)
        return false;
    const def = petById(runtime.defId);
    if (def === null)
        return false;
...
export function handlePetAbility(ctx, userId, abilityId) {
    switch (abilityId) {
...
export function isPetAbility(abilityId) {
    return Object.values(PET_ABILITY_IDS).includes(abilityId);
}
export function isPetInstance(ctx, instanceId) {
...
export function tickPets(ctx, userId, dt) {
    const runtime = petsOf(ctx).get(userId);
    if (runtime === undefined || runtime.instanceId === null || runtime.dead)
        return;
    const def = petById(runtime.defId);
    ...
    let targetId = ctx.scene.entity.getTarget(userId);
    ...
    if (targetId !== null) {
        ...
        if (dist !== null && dist > 2.5) {
            ...
            if (now >= runtime.nextSwingAt) {
                ...
                if (def.role === "tank")
                    addThreat(ctx, targetId, runtime.instanceId, amount * 2);
...
export function resetPets(ctx, userId) {
    if (userId !== undefined) {
