...
function billboard(body, accent, outline) {
...
function familyShape(mobId, family) {
    if (/spider|widow/.test(mobId))
        return "spider";
    if (/boar/.test(mobId))
        return "boar";
    if (/rat/.test(mobId))
        return "rat";
    if (/murloc|mudfin|deepfen|bloat/.test(mobId))
        return "murloc";
    if (/troll|ogre/.test(mobId))
        return "brute";
    if (/kobold|sapper/.test(mobId))
        return "kobold";
    if (/elemental|storm/.test(mobId))
        return "elemental";
    if (/cultist|acolyte|necromancer|zealot/.test(mobId))
        return "cultist";
    if (/ghost|drowned|shambler/.test(mobId))
        return "ghost";
    if (/bones|marrow|crypt/.test(mobId))
        return "skeleton";
    if (/bandit/.test(mobId))
        return "bandit";
    if (family === "undead")
        return "skeleton";
    if (family === "elemental")
...
for (const npc of NPCS) {
    entitySprites[npc.id] = {
        url: billboard(npc.kind === "vendor" ? SHAPES.vendor : SHAPES.guard, npc.kind === "vendor" ? "#e8c15a" : "#8fb4e0", "#3d3a34"),
        width: 1.7,
        height: 1.9,
        y: 1.0,
    };
