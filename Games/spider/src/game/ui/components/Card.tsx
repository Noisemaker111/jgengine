...
import { color, RANK_LABEL, SUIT_GLYPH, type Card, type Suit } from "../../spider/engine";
...
function cornerStyle(metrics: CardMetrics, ink: string, rotated: boolean): CSSProperties {
  const size = Math.max(10, Math.round(metrics.w * 0.3));
  return {
    position: "absolute",
    ...(rotated
      ? { right: Math.round(metrics.w * 0.08), bottom: Math.round(metrics.h * 0.05) }
      : { left: Math.round(metrics.w * 0.08), top: Math.round(metrics.h * 0.04) }),
    display: "flex",
  ...
  return (
    <div
      style={{
        ...
        boxShadow: lifted
          ? "0 12px 26px rgba(0,0,0,0.55), inset 0 0 0 1px rgba(255,255,255,0.7)"
          : "0 1.5px 3px rgba(0,0,0,0.4), inset 0 0 0 1px rgba(255,255,255,0.6)",
        position: "relative",
        opacity: dim ? 0.32 : 1,
        userSelect: "none",
        touchAction: "none",
  ...
  return (
    <div
      style={{
        ...
        height: metrics.h,
        borderRadius: r,
        background: "repeating-linear-gradient(45deg,#6e1f33 0 7px,#7d2740 7px 14px)",
        border: "2px solid #c9ccd6",
        boxShadow: "0 1.5px 3px rgba(0,0,0,0.45), inset 0 0 0 1px rgba(0,0,0,0.25)",
        position: "relative",
        boxSizing: "border-box",
        userSelect: "none",
      }}
    >
      <div
        style={{
          ...
          border: "1px solid rgba(201,204,214,0.7)",
        }}
      />
      <div
        style={{
...
export function EmptySlot({
  ...
  const tint = suit !== undefined && color(suit) === "red" ? "rgba(226,150,158,0.5)" : "rgba(226,224,232,0.4)";
  return (
    <div
      style={{
        width: metrics.w,
        height: metrics.h,
        borderRadius: radius(metrics),
        border: `2px ${dashed ? "dashed" : "solid"} rgba(226,224,232,0.32)`,
