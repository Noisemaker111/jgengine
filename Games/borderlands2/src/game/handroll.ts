...
const MANUFACTURERS: readonly Manufacturer[] = [
  { id: "Jakobs", damage: 1.3, interval: 1.25, spread: 0.9, mag: 0.8, reload: 1.1, neverElemental: true, prefixes: ["Cowboy", "Buffalo", "Doc's", "Frontier"] },
  { id: "Hyperion", damage: 1, interval: 1, spread: 0.6, mag: 1, reload: 0.95, prefixes: ["Corporate", "Crowdsourced", "Visionary", "Future"] },
  { id: "Maliwan", damage: 0.95, interval: 1, spread: 0.85, mag: 1, reload: 1, forcedElement: undefined, prefixes: ["Pyrotechnic", "Refined", "Proactive", "Elegant"] },
  { id: "Tediore", damage: 0.9, interval: 0.95, spread: 1.05, mag: 0.9, reload: 0.45, prefixes: ["Bargain", "Value", "Practical", "Budget"] },
  { id: "Torgue", damage: 1.2, interval: 1.15, spread: 1.15, mag: 1, reload: 1.15, forcedElement: "explosive", prefixes: ["EXPLOSIVE", "Ferocious", "Original", "Wild"] },
  { id: "Vladof", damage: 0.85, interval: 0.6, spread: 1.1, mag: 1.3, reload: 1, prefixes: ["Patriot's", "Rabid", "Worker's", "Surplus"] },
  { id: "Dahl", damage: 0.95, interval: 0.9, spread: 0.75, mag: 1, reload: 0.9, prefixes: ["Military", "Operational", "Scout's", "Stoic"] },
  { id: "Bandit", damage: 1.05, interval: 1.05, spread: 1.3, mag: 1.7, reload: 1.25, prefixes: ["Murduring", "Extra Big", "Bonus", "Stabbing"] },
];
...
const LEGENDARY_NAMES: Record<GunFamily, readonly (readonly [string, string])[]> = {
  pistol: [["Torgue", "Unkempt Harold"], ["Vladof", "Infinity"], ["Jakobs", "Maggie"]],
  smg: [["Maliwan", "Hellfire"], ["Bandit", "Slagga"], ["Hyperion", "Bitch"]],
  shotgun: [["Hyperion", "Conference Call"], ["Torgue", "Flakker"], ["Bandit", "Sledge's Shotgun"]],
  rifle: [["Vladof", "Shredifier"], ["Jakobs", "Hammer Buster"], ["Dahl", "Veruc"]],
  sniper: [["Vladof", "Lyuda"], ["Maliwan", "Volcano"], ["Jakobs", "Skullmasher"]],
  launcher: [["Bandit", "Badaboom"], ["Maliwan", "Norfleet"], ["Torgue", "Nukem"]],
};

const ELEMENTS: readonly GunElement[] = ["incendiary", "shock", "corrosive", "slag"];
...
export function rollGun(rng: () => number, level: number, options: RollGunOptions = {}): GunDef {
  ...
  if (manufacturer.neverElemental !== true) {
    ...
    else if (manufacturer.id === "Maliwan" || rng() < tier.elementChance) element = pick(rng, ELEMENTS);
  }
...
const slaggedUntil = new Map<string, number>();

export const DOT_DURATION_MS = 4000;
export const SLAG_DURATION_MS = 8000;
export const SLAG_DAMAGE_MULT = 2;

export function isSlagged(targetId: string, nowMs: number): boolean {
  return (slaggedUntil.get(targetId) ?? 0) > nowMs;
}
...
export function elementalDamageMult(
  ...
  if (isSlagged(targetId, nowMs) && element !== "slag") mult *= SLAG_DAMAGE_MULT;
  return mult;
}
