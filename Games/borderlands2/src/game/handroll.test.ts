...
describe("gun generation", () => {
  ...
  test("legendary guns carry real legendary names and tighter spread", () => {
    ...
    expect(["Lyuda", "Volcano", "Skullmasher"].some((name) => gun.name.includes(name))).toBe(true);
  });

  test("jakobs guns never spawn elemental", () => {
    const rng = seededRng("jakobs-test");
    for (let index = 0; index < 300; index += 1) {
      const gun = rollGun(rng, 5);
      if (gun.manufacturer === "Jakobs") expect(gun.element === "none" || gun.element === "explosive").toBe(true);
    }
  });
