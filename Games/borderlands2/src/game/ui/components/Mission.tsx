  ...
  if (active.length === 0) {
    return (
      <div className="max-w-[16rem] border-l-4 border-amber-400 bg-black/60 px-3 py-2 text-[11px] font-bold uppercase tracking-wider text-stone-300">
        All missions complete — Pandora is (briefly) quiet
      </div>
    );
  }
  return (
...
export function CashPlate() {
  ...
  return (
    <div className="flex skew-x-[-8deg] items-baseline gap-1 border-2 border-black/80 bg-black/70 px-3 py-1">
      <span className="text-sm font-black" style={{ color: PANDORA.hudXp }}>
        $
      </span>
...
export function CreditBadge() {
  return (
    <div className="pointer-events-auto flex items-center gap-1.5 rounded-sm bg-black/50 px-2 py-0.5 text-[9px] font-semibold uppercase tracking-wider text-stone-400">
      <img
        src="https://unavatar.io/x/GearboxOfficial"
        alt=""
      ...
      />
      <a href="https://www.borderlands.com" target="_blank" rel="noreferrer" className="hover:text-amber-300">
        Fan demake homage to Gearbox's Borderlands 2
      </a>
    </div>
  );
}
