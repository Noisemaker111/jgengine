...
- Record consumer-facing changes as you make them. A PR that changes `packages/*/src` adds its line to `CHANGELOG.md` under `## [Unreleased]`, so the next publish's notes are complete without an archaeology pass; at release the heading is renamed to the version and mirrored into `packages/core/src/meta/changelog.ts`. `bun run check-changelog` gates this in CI (in the PR `quick` job); a pure refactor or test-only change bypasses with `[skip changelog]` in a commit message.
