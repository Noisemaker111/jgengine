...
/**
 * `?look=` → the shipped `observer` rig bound to that point with the orbit stopped. Without this a
 * capture can only be aimed by moving the player and hoping: the spawn override does not pin look
...
function lookCamera(game: PlayableGame, markers: LookMarkers | undefined): GameCameraConfig | undefined {
  ...
  if (resolution.kind === "error") {
    ...
    // the host reports this reason instead of a load-error stack wrapped around it.
    const message = `look override rejected: ${resolution.message}`;
    setCaptureStatus("error", message);
    console.error(`[jgengine:capture] ${message}`);
    return undefined;
  }
  const summary = formatAppliedAim(resolution.applied);
  document.documentElement.dataset.jgLook = summary;
  console.debug("[jgengine:capture] look", summary);
...
export const gameRegistry: GameRegistry = {
  ...
  "key-rebinding": () => import("./demo/keyRebindingDemo").then((module) => module.keyRebindingDemoGame),
